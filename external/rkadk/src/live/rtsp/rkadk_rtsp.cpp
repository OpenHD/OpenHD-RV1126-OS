...
static RKADK_S32 RKADK_RTSP_RequestIDR(RKADK_U32 u32CamId, RKADK_U32 u32ChnId) {
  ...
  RKADK_STREAM_TYPE_E enType =
      RKADK_PARAM_VencChnMux(u32CamId, u32ChnId, RKADK_STREAM_TYPE_LIVE);
...
static bool RKADK_RTSP_IsUseRga(RKADK_U32 u32CamId,
                                RKADK_PARAM_STREAM_CFG_S *pstLiveCfg) {
  bool bUseRga = false;
...
RKADK_S32 RKADK_RTSP_Init(RKADK_U32 u32CamId, RKADK_U32 port, const char *path,
  ...
  if (enType != RKADK_STREAM_TYPE_BUTT) {
    switch (enType) {
  ...
  // Create RGA
  bUseRga = RKADK_RTSP_IsUseRga(u32CamId, pstLiveCfg);
  if (bUseRga) {
...
RKADK_S32 RKADK_RTSP_DeInit(RKADK_MW_PTR pHandle) {
  ...
  bUseRga = RKADK_RTSP_IsUseRga(pstHandle->u32CamId, pstLiveCfg);
