...
static void RKADK_PARAM_SetMicVolume(RKADK_U32 volume) {
  ...
  sprintf(buffer, "amixer cset iface=MIXER,name='Master Capture Volume' %d%%,%d%%", volume, volume);
  system(buffer);
}
...
static void RKADK_PARAM_SetSpeakerVolume(RKADK_U32 volume) {
  ...
  memset(buffer, 0, RKADK_BUFFER_LEN);
  sprintf(buffer, "amixer cset iface=MIXER,name='Master Playback Volume' %d%%,%d%%", volume, volume);
  system(buffer);
}
...
static void RKADK_PARAM_MicMute(bool mute, RKADK_U32 volume) {
  ...
  memset(buffer, 0, RKADK_BUFFER_LEN);
  if (mute)
    sprintf(buffer, "amixer cset iface=MIXER,name='Master Capture Volume' %d%%,%d%%", 0, 0);
  else
    sprintf(buffer, "amixer cset iface=MIXER,name='Master Capture Volume' %d%%,%d%%", volume, volume);
...
RKADK_S32 RKADK_PARAM_GetRgaChnId(RKADK_U32 u32CamId,
                                  RKADK_STREAM_TYPE_E enStrmType) {
  RKADK_S32 s32RgaChnId = -1;
  ...
  case RKADK_STREAM_TYPE_VIDEO_MAIN: {
    ...
    if (!pstRecCfg)
      ...
      s32RgaChnId = pstRecCfg->attribute[0].rga_chn;
    break;
  }
  case RKADK_STREAM_TYPE_VIDEO_SUB: {
    RKADK_PARAM_REC_CFG_S *pstRecCfg = RKADK_PARAM_GetRecCfg(u32CamId);
    if (!pstRecCfg)
      ...
      s32RgaChnId = pstRecCfg->attribute[1].rga_chn;
    break;
  }
  case RKADK_STREAM_TYPE_SNAP: {
    RKADK_PARAM_PHOTO_CFG_S *pstPhotoCfg = RKADK_PARAM_GetPhotoCfg(u32CamId);
    if (!pstPhotoCfg)
      ...
      s32RgaChnId = pstPhotoCfg->rga_chn;
    break;
  }
  case RKADK_STREAM_TYPE_PREVIEW:
  case RKADK_STREAM_TYPE_LIVE: {
    ...
    if (!pstStreamCfg)
      ...
      s32RgaChnId = pstStreamCfg->attribute.rga_chn;
    break;
  }
  case RKADK_STREAM_TYPE_DISP: {
    ...
    if (!pstDispCfg)
      ...
      s32RgaChnId = pstDispCfg->rga_chn;
    break;
  }
  default:
    RKADK_LOGE("Unsupport stream type: %d", enStrmType);
...
RKADK_STREAM_TYPE_E RKADK_PARAM_VencChnMux(RKADK_U32 u32CamId,
  ...
  if (enType != RKADK_STREAM_TYPE_PREVIEW) {
    RKADK_PARAM_STREAM_CFG_S *pstStreamCfg =
  ...
  if (enType != RKADK_STREAM_TYPE_LIVE) {
