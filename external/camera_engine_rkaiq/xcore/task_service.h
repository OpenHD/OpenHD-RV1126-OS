...
class TaskService {
 public:
    ...
    void loop() {
        ...
        while (started_) {
...

            {
                ...
                // of params
                out_params_.push_front(std::move(param));
    ...
    constexpr uint8_t getMaxParamCount() { return max_param_count_; }
    void setMaxParamCount(const uint8_t count) { max_param_count_ = count; }

    constexpr TaskDuration getMaxProceedTime() { return max_process_time_; }
