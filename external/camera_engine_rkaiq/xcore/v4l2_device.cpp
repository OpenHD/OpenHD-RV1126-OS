...
#define XCAM_V4L2_DEFAULT_BUFFER_COUNT  7
...
V4l2Device::open (bool block)
{
    ...
    _fd = ::open (_name, flags);
    if (_fd == -1) {
        XCAM_LOG_ERROR ("open device(%s) failed, error:%s", _name, strerror(errno));
...
int
V4l2Device::poll_event (int timeout_msec, int stop_fd, bool is_event)
{
    ...
    auto events = (POLLERR | POLLNVAL | POLLHUP);
...
V4l2Device::stop ()
{
    ...
    XCAM_LOG_INFO ("device(%s) stop, already start: %d", XCAM_STR (_name), _active.load());
...
V4l2Device::fini_buffer_pool()
{
    ...
    if (_memory_type == V4L2_MEMORY_MMAP) {
        ...
        while (1) {
            ret_b = io_control (VIDIOC_REQBUFS, &request_buf);
            if (ret_b >= 0 || errno != EBUSY)
                break;
            if (++tried_time > 50)
                break;
            usleep (100);
        ...
        if (ret_b < 0) {
            XCAM_LOG_ERROR ("device(%s) stop failed on VIDIOC_REQBUFS, error:%s",
                    XCAM_STR (_name), strerror(errno));
...
V4l2Device::get_buffer (SmartPtr<V4l2Buffer> &buf, int index) const
{
    ...
    if (index >= int(_buf_pool.size())) {
        XCAM_LOG_ERROR("get_buffer failed index %d out of bounds %d", index,
                       _buf_pool.size());
        return XCAM_RETURN_ERROR_PARAM;
    }
