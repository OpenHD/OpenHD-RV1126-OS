...
RkAiqMngCmdThread::loop ()
{
    ...
    case MSG_CMD_SW_WORKING_MODE:
        ...
        break;
    case MSG_CMD_SW_STREAM_STATE:
        if (msg->sync) msg->mutex->lock();
        if (msg->data.sw_stream_state.start) {
            mAiqMng->start();
        } else {
            mAiqMng->stop();
        }
...
XCamReturn
RkAiqManager::set_read_back_mode(bool on)
{
    ENTER_XCORE_FUNCTION();
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    ret = mCamHw->set_read_back_mode(on);
    EXIT_XCORE_FUNCTION();
    return ret;
}
...
RkAiqManager::init()
{
    ...
    mCamHw->setNrStatsListener(this);
...
XCamReturn RkAiqManager::setTxBufNum(uint16_t buf_num)
{
    ENTER_XCORE_FUNCTION();
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    ret = mCamHw->set_tx_buf_num(buf_num);
    EXIT_XCORE_FUNCTION();
    return ret;
}
...
RkAiqManager::prepare(uint32_t width, uint32_t height, rk_aiq_working_mode_t mode)
{
    ...
    // get hw working mode, online or offline,
    // awb need know this now
    mCamHw->getIspWorkMode(working_mode_hw);
...
RkAiqManager::start()
{
    ...
    if (_state == AIQ_STATE_STOPED) {
        ...
        if (initParams->data()->mIspOtherParams.ptr()) {
            initParams->data()->mIspOtherParams->data()->frame_id = 0;
        }
        ...
        if (initParams->data()->mIsppOtherParams.ptr()) {
            initParams->data()->mIsppOtherParams->data()->frame_id = 0;
        }

        if (initParams->data()->mTnrParams.ptr()) {
            initParams->data()->mTnrParams->data()->frame_id = 0;
        }

        if (initParams->data()->mFecParams.ptr()) {
            initParams->data()->mFecParams->data()->frame_id = 0;
        }
...
RkAiqManager::ispEvtsCb(SmartPtr<ispHwEvt_t> evts)
{
    ...
    if (evts->evt_code == V4L2_EVENT_FRAME_SYNC) {
        ret = mRkAiqAnalyzer->pushEvts(evts);
    } else if (evts->evt_code == CIFISP_V4L2_EVENT_STREAM_START) {
        LOGE_ANALYZER("========================== stream start =======================\n");
        SmartPtr<RkAiqMngCmdThread::msg_t> msg = new RkAiqMngCmdThread::msg_t();
        msg->cmd                               = RkAiqMngCmdThread::MSG_CMD_SW_STREAM_STATE;
        msg->sync                              = false;
        msg->data.sw_stream_state.start        = true;
        mAiqMngCmdTh->send_cmd(msg);
    } else if (evts->evt_code == CIFISP_V4L2_EVENT_STREAM_STOP) {
        LOGE_ANALYZER("========================== stream stop =======================\n");
        SmartPtr<RkAiqMngCmdThread::msg_t> msg = new RkAiqMngCmdThread::msg_t();
        msg->cmd                               = RkAiqMngCmdThread::MSG_CMD_SW_STREAM_STATE;
        msg->sync                              = false;
        msg->data.sw_stream_state.start        = false;
        mAiqMngCmdTh->send_cmd(msg);
    }
    return ret;
}
...
XCamReturn
RkAiqManager::nrStatsCb(SmartPtr<VideoBuffer>& nrStats)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    uint64_t timestamp = 0;
    SmartPtr<RkAiqExpParamsProxy> expParams = nullptr;

    ret = mCamHw->getEffectiveExpParams(nrStats->get_sequence(), expParams, timestamp);
    if (ret < 0)
        LOGE_ANALYZER("id %d, Failed to get expParam: %d!",  nrStats->get_sequence(), ret);

    ret = mRkAiqAnalyzer->pushEisInputInfo(nrStats, expParams, timestamp);
    return ret;
}
...
RkAiqManager::applyAnalyzerResult(SmartPtr<RkAiqFullParamsProxy>& results)
{
    ...
    std::lock_guard<std::mutex> lg(mCfgLock);
    ...
    if (aiqParams->mCpslParams.ptr()) {
        ...
        int gray_mode = aiqParams->mIspMeasParams->data()->ie.base.mode;
...
set_isp_end:
    ...
    if (aiqParams->mIsppOtherParams.ptr()) {
        aiqParams->mIsppOtherParams->setId (aiqParams->mIsppOtherParams->data()->frame_id);
        aiqParams->mIsppOtherParams->setType (RESULT_TYPE_ANR);
    ...
    if (aiqParams->mIsppMeasParams.ptr()) {
        aiqParams->mIsppMeasParams->setId (aiqParams->mIsppMeasParams->data()->frame_id);
        aiqParams->mIsppMeasParams->setType (RESULT_TYPE_AORB);
    ...
    if (aiqParams->mTnrParams.ptr ()) {
        aiqParams->mTnrParams->setId (aiqParams->mTnrParams->data()->frame_id);
        aiqParams->mTnrParams->setType (RESULT_TYPE_ATNR);
        ret = mCamHw->setTnrParams (aiqParams->mTnrParams);
        if (ret)
            LOGE_ANALYZER("set tnr params error %d", ret);
    }

    if (aiqParams->mFecParams.ptr ()) {
        aiqParams->mFecParams->setId (aiqParams->mFecParams->data()->frame_id);
        aiqParams->mFecParams->setType (RESULT_TYPE_AFEC);
        ret = mCamHw->setFecParams (aiqParams->mFecParams);
        if (ret)
            LOGE_ANALYZER("set fec params error %d", ret);
    }
...
XCamReturn
RkAiqManager::get_rkraw_frame_info(rkraw_frame_info_t *frame_info)
{
    ENTER_XCORE_FUNCTION();
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    ret = mCamHw->get_rkraw_frame_info(frame_info);
    EXIT_XCORE_FUNCTION();
    return ret;
}
