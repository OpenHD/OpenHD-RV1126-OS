...
prepare(RkAiqAlgoCom* params)
{
    ...
    int working_mode;
    int oldeParamMode;
    oldeParamMode = hAlut3d->eParamMode;
    working_mode = params->u.prepare.working_mode;
    if(working_mode == RK_AIQ_WORKING_MODE_NORMAL) {
        hAlut3d->eParamMode = ALut3d_PARAM_MODE_NORMAL;
    } else if(working_mode >= RK_AIQ_WORKING_MODE_ISP_HDR2
             &&  working_mode <= RK_AIQ_WORKING_MODE_ISP_HDR3 ) {
       hAlut3d->eParamMode = ALut3d_PARAM_MODE_HDR;
    } else{
        LOGE_A3DLUT("%s: Current working mode (%d) is invalid!Defaults to normal mode.  \n", __FUNCTION__, working_mode);
        hAlut3d->eParamMode = ALut3d_PARAM_MODE_NORMAL;
    }
    if(oldeParamMode!=hAlut3d->eParamMode){
        hAlut3d->prepare_type |=RK_AIQ_ALGO_CONFTYPE_UPDATECALIB;
        LOGI_A3DLUT("hdr(1)/linear(0) mode change from %d to %d",oldeParamMode,hAlut3d->eParamMode);
    }
