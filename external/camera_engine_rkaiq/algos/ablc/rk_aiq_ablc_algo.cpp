...
AblcResult_t Ablc_config_mode_param(AblcContext_t *pAblcCtx, CalibDb_Blc_t* pBlcCalib, AblcParamMode_t eParamMode)
{
    ...
    memcpy(pAblcCtx->mode_name, mode_name, sizeof(mode_name));
    ...
    Ablc_xml_params_init(&pAblcCtx->stAuto.stParams, pBlcCalib, mode_idx);
...
AblcResult_t AblcInit(AblcContext_t **ppAblcCtx, CamCalibDbContext_t *pCalibDb)
{
    ...
    *ppAblcCtx = pAblcCtx;
    ...
    Ablc_config_mode_param(pAblcCtx, &pAblcCtx->stBlcCalib, pAblcCtx->eParamMode);
#else
    //static init params
