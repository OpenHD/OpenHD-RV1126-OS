...
#if 1
    //real use
    interpolation(pCcm->mode_cell[mode_idx].luma_ccm.alpha_gain,
                  pCcm->mode_cell[mode_idx].luma_ccm.alpha_scale,
                  pCcm->mode_cell[mode_idx].luma_ccm.gain_scale_cure_size,
                  sensorGain, &fScale);
#else
...
(
    ...
    if(hAccm->updateAtt) {
        ...
        if (hAccm->mCurAtt.mode == RK_AIQ_CCM_MODE_TOOL)
        {
            ...
            hAccm->ccmHwConf.bound_bit = hAccm->mCurAtt.stTool.mode_cell[hAccm->accmRest.currentHdrNormalMode].luma_ccm.low_bound_pos_bit;
            memcpy(hAccm->ccmHwConf.rgb2y_para, hAccm->mCurAtt.stTool.mode_cell[hAccm->accmRest.currentHdrNormalMode].luma_ccm.rgb2y_para,
                   sizeof(hAccm->mCurAtt.stTool.mode_cell[hAccm->accmRest.currentHdrNormalMode].luma_ccm.rgb2y_para));

            if(hAccm->mCurAtt.byPass != true && hAccm->accmSwInfo.grayMode != true) {
                ret = pCcmMatrixAll_init(hAccm, &hAccm->mCurAtt.stTool.mode_cell[hAccm->accmRest.currentHdrNormalMode], hAccm->accmRest.currentHdrNormalMode);
                RETURN_RESULT_IF_DIFFERENT(ret, XCAM_RETURN_NO_ERROR);
            }
        }
    ...
    if(hAccm->mCurAtt.byPass != true && hAccm->accmSwInfo.grayMode != true) {
        ...
        if((hAccm->mCurAtt.mode == RK_AIQ_CCM_MODE_AUTO)|| (hAccm->mCurAtt.mode == RK_AIQ_CCM_MODE_TOOL)){
            ...
            hAccm->mCurAtt.stTool.mode_cell[currentHdrNormalMode].luma_ccm.low_bound_pos_bit = hAccm->ccmHwConf.bound_bit;
            memcpy(hAccm->mCurAtt.stTool.mode_cell[currentHdrNormalMode].luma_ccm.y_alpha_curve, hAccm->ccmHwConf.alp_y, sizeof(hAccm->ccmHwConf.alp_y));
        }
        else {
