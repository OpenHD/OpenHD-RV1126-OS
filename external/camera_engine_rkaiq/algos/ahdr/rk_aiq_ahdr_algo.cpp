...
(
    ...
    pAhdrCtx->AhdrPrevData.PrevTmoHandleData.GlobeLuma = 185;
    pAhdrCtx->AhdrPrevData.PrevTmoHandleData.GlobeMaxLuma = 308;
    pAhdrCtx->AhdrPrevData.PrevTmoHandleData.DetailsHighLight = 512;
    pAhdrCtx->AhdrPrevData.PrevTmoHandleData.DetailsLowLight = 16;
    pAhdrCtx->AhdrPrevData.PrevTmoHandleData.LocalTmoStrength = 76;
...
(
    ...
    LOG1_AHDR("%s:  Current CurrL2S_Ratio:%f CurrL2M_Ratio:%f CurrL2L_Ratio:%f\n", __FUNCTION__, pAhdrCtx->CurrHandleData.CurrL2S_Ratio,
...
(
    ...
    if(GlobalLuma_mode == 0) {
        ...
        //fix tmo GlobeLuma GlobeMaxLuma
        pAhdrCtx->CurrHandleData.CurrTmoHandleData.GlobeLuma = pAhdrCtx->AhdrConfig.tmo_para.Luma.GlobeLuma[12];
    }
        ...
        pAhdrCtx->CurrHandleData.CurrTmoHandleData.GlobeLuma = GetCurrPara(pAhdrCtx->CurrHandleData.CurrISO,
                ...
                pAhdrCtx->AhdrConfig.tmo_para.Luma.GlobeLuma);
        //fix tmo GlobeLuma GlobeMaxLuma
        pAhdrCtx->CurrHandleData.CurrTmoHandleData.GlobeLuma = pAhdrCtx->AhdrConfig.tmo_para.Luma.GlobeLuma[0];
    }
...
(
    ...
    //get info
    pAhdrCtx->CurrAeResult.IsConverged = AecHdrProcResult.IsConverged;
...
(
    ...
    pAhdrCtx->AhdrConfig.tmo_para.AeConvergedPdf = LIMIT_VALUE(pCalibDb->tmo.AeConvergedPdf, DAMPMAX, DAMPMIN);
...
)
{
    ...
    if(pAhdrCtx->hdrAttr.opMode == HDR_OpMode_Api_OFF)
    {
        ...
        //log after updating
        if(pAhdrCtx->FrameNumber > 1 && pAhdrCtx->FrameNumber <= 3) {
    ...
    else if(pAhdrCtx->hdrAttr.opMode == HDR_OpMode_DarkArea)
    {
        ...
        pAhdrCtx->CurrHandleData.CurrTmoHandleData.DetailsLowLight *= 1 + (float)(pAhdrCtx->hdrAttr.stDarkArea.level) * 0.04;
        pAhdrCtx->CurrHandleData.CurrTmoHandleData.DetailsLowLight =
            LIMIT_VALUE(pAhdrCtx->CurrHandleData.CurrTmoHandleData.DetailsLowLight, DETAILSLOWLIGHTMAX, DETAILSLOWLIGHTMIN);
    ...
    LOGD_AHDR("%s:  preFrame lgMergeLuma:%f MergeLuma(8bit):%f SLuma(8bit):%d MLuma(8bit):%d LLuma(8bit):%d TmoLuma(8bit):%d\n", __FUNCTION__,
              lgmean, MergeLuma, short_mean, middle_mean, long_mean, tmo_mean);
...
(
    ...
    memcpy(&pAhdrCtx->pCalibDB, &pCalib->ahdr, sizeof(CalibDb_Ahdr_Para_t));//load iq paras
    memcpy(&pAhdrCtx->hdrAttr.stTool, &pCalib->ahdr, sizeof(CalibDb_Ahdr_Para_t));//load iq paras to stTool
