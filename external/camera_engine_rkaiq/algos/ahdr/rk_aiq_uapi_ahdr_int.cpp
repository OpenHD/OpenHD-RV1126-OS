...
XCamReturn
rk_aiq_ahdr_OverWriteCalibByCurAttr
(
    ...
    AhdrInstanceConfig_t* pAhdrInstConfig = (AhdrInstanceConfig_t*)ctx;
    AhdrHandle_t pAhdrCtx = pAhdrInstConfig->hAhdr;

    if(pAhdrCtx->hdrAttr.opMode >= HDR_OpMode_Api_OFF || pAhdrCtx->hdrAttr.opMode < HDR_OpMode_Tool)
        memcpy(ahdr, &pAhdrCtx->pCalibDB, sizeof(CalibDb_Ahdr_Para_t));
    else if(pAhdrCtx->hdrAttr.opMode == HDR_OpMode_Tool)
        memcpy(ahdr, &pAhdrCtx->hdrAttr.stTool, sizeof(CalibDb_Ahdr_Para_t));
    else
