...
ANRresult_t mfnr_get_setting_idx_by_name(CalibDb_MFNR_2_t *pCalibdb, const char *name, int mode_idx, int *setting_idx)
{
    int i = 0;
    ANRresult_t res = ANR_RET_SUCCESS;
...
ANRresult_t mfnr_config_motion_param(CalibDb_MFNR_Motion_V2_t *pMotion,  CalibDb_MFNR_2_t *pCalibdb, char* param_mode)
{
    ...
    memcpy(pMotion, &pCalibdb->mode_cell[mode_idx].motion, sizeof(CalibDb_MFNR_Motion_V2_t));
...
ANRresult_t init_mfnr_params(RKAnr_Mfnr_Params_t *pParams, CalibDb_MFNR_2_t *pCalibdb, int mode_idx, int setting_idx)
{
    ...
    pParams->dynamicEnable = pCalibdb->mode_cell[mode_idx].motion.dynamicEnable;
    pParams->dynamicSw[0] = pCalibdb->mode_cell[mode_idx].motion.dynamicSw[0];
    pParams->dynamicSw[1] = pCalibdb->mode_cell[mode_idx].motion.dynamicSw[1];
    for(int j = 0; j < MAX_ISO_STEP; j++)
    {
        pParams->mfnr_sigma_scale[j] = pCalibdb->mode_cell[mode_idx].motion.mfnrSigmaScale[j];
    }

    pParams->gain_upt_flg    = pCalibdb->mode_cell[mode_idx].motion.gainUpdFlag;
#endif
    LOGD_ANR("pCalibdb->mode_cell[%d].motion.enable %d pCalibdb->motion_detect_en %d pParams->motion_detection_enable %d %x\n",
             mode_idx,
             pCalibdb->mode_cell[mode_idx].motion.enable,
             pCalibdb->motion_detect_en,
             pParams->motion_detection_enable,
             &pParams->motion_detection_enable);
...
ANRresult_t select_mfnr_params_by_ISO(RKAnr_Mfnr_Params_t *stmfnrParams,    RKAnr_Mfnr_Params_Select_t *stmfnrParamsSelected, ANRExpInfo_t *pExpInfo, int bits_proc)
{
    ...
    float gain                                      = iso / 50;
    float dynamicSw                                 = 0.0;
    int   motion_detect_control                     = 1;
    static int   motion_detection_status            = 0;

    LOGD_ANR("MFNR:current gain is %f motion_status = %d \n",gain,motion_detection_status);
    if (stmfnrParams->dynamicEnable) {
        if (motion_detection_status)
            dynamicSw = stmfnrParams->dynamicSw[0];
        else
            dynamicSw = stmfnrParams->dynamicSw[1];
        if (gain > dynamicSw){
            motion_detect_control = 1;
            motion_detection_status = 1;
            LOGD_ANR("MFNR: Switch on dynamicSw = %f\n ",dynamicSw);
        } else {
            motion_detect_control= 0;
            motion_detection_status = 0;
            LOGD_ANR("MFNR: Switch off dynamicSw = %f\n ",dynamicSw);
        }
    }
    ...
    if(stmfnrParams->motion_detection_enable == 0)
        stmfnrParams->gain_upt_flg    = 0;


    static int num = 0;
    if(0)//(num%90)==0)
        LOGD_ANR("mfnr motion detetion enable:%d upt %d mfnr_sigma_scale:%f %x\n",
                 stmfnrParams->motion_detection_enable, stmfnrParams->gain_upt_flg,
                 stmfnrParamsSelected->mfnr_sigma_scale, &stmfnrParams->motion_detection_enable);
    if(stmfnrParams->motion_detection_enable && stmfnrParams->gain_upt_flg && motion_detect_control) {
...
//    noise_sigma_tmp[i]                          = ROUND_F(noise_sigma_tmp[i] * (1 << 12)) >>  ;
    ...
    num++;
    if((num % 600) == 0)
        LOGD_ANR("mfnr final sigma_max:%f motion_scale:%f sigma_limit:%f sigma_scale:%f scale_scale:%f upt flg %d\n",
                 noise_sigma_max,
                 stmfnrParamsSelected->mfnr_sigma_scale,
                 noise_sigma_limit,
                 sigma_scale,
                 scale_scale,
                 stmfnrParams->gain_upt_flg);
