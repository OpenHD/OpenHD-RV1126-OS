...
ANRresult_t ANRGetProcResult(ANRContext_t *pANRCtx, ANRProcResult_t* pANRResult)
{
    ...
    if(pANRCtx->eMode == ANR_OP_MODE_AUTO) {
        ...
        if(pANRCtx->stAuto.mfnrEn && pANRCtx->stAuto.stMfnr_dynamic.enable && !pANRCtx->stMfnrCalib.motion_detect_en) {
            pANRResult->mfnrEn = pANRCtx->stAuto.stMfnr_dynamic.mfnr_enable_state;
            LOGD_ANR("dynamic mfnr en:%d\n", pANRResult->mfnrEn);
        }
    ...
    if(pANRCtx->stMfnrCalib.motion_detect_en == 1) {
        pANRResult->stMfnrFix.mode = 0;
    } else {
	...
	int setting_idx = 0;
	if(pANRCtx->stExpInfo.snr_mode == 1) {
        mfnr_get_setting_idx_by_name(&pANRCtx->stMfnrCalib, "HSNR", mode_idx, &setting_idx);
    } else{
        mfnr_get_setting_idx_by_name(&pANRCtx->stMfnrCalib, "LSNR", mode_idx, &setting_idx);
    } 
    ...
    {
        for(int j = 0; j < 5; j++)
            pANRResult->stMotionParam.noise_curve[i][j] = pANRCtx->stMfnrCalib.mode_cell[mode_idx].setting[setting_idx].mfnr_iso[i].noise_curve[j];
        pANRResult->stMotionParam.noise_curve[i][5] = pANRCtx->stMfnrCalib.mode_cell[mode_idx].setting[setting_idx].mfnr_iso[i].noise_curve_x00;
    }
    LOGI_ANR("%s(%d): exit!\n", __FUNCTION__, __LINE__);
    return ANR_RET_SUCCESS;
}
...
ANRresult_t ANRConfigSettingParam(ANRContext_t *pANRCtx, ANRParamMode_t eParamMode, int snr_mode)
{
    ...
    memcpy(pANRCtx->param_mode_name, param_mode_name, sizeof(param_mode_name));
    memcpy(pANRCtx->snr_name, snr_name, sizeof(snr_name));
    ...
    mfnr_config_motion_param(&pANRCtx->stAuto.stMfnr_Motion, &pANRCtx->stMfnrCalib, param_mode_name);
