...
FecMeshBuffer* FecRemapBackend::GetPendingHwResult() {
        ...
        std::for_each(user_buffers_.begin(), user_buffers_.end(),
                      [&](const std::unique_ptr<FecMeshBuffer>& p) {
                          if (p->State[0] == fec_hw_mesh_used_by_hardware ||
                              ...
                              LOGD_AEIS(">>>>>>>>> %d", p->FrameId);
                              if (p->FrameId != 0 && p->FrameId <= last_result_id_) {
                                  LOGD_AEIS("Get pending result id %d<%d PASSED!!!", p->FrameId, last_result_id_);
                                  p->State[0] = fec_mesh_available;
                              } else if (p->FrameId - last_result_id_ > 1) {
                                  LOGD_AEIS("pending result id %d in FUTURE!!!", p->FrameId);
                              } else {
                                  if (min_id >= p->FrameId) {
                                      min_id = p->FrameId;
                                      buf    = p.get();
    ...
    LOGD_AEIS("Get Pending result min id %d", min_id);

    if (buf != nullptr) {
        if (buf->FrameId - last_result_id_ > 1) {
            LOGD_AEIS("Get pending result id %d FUTURE ???", buf->FrameId);
            return nullptr;
        }
        if (buf->State[0] == fec_mesh_skipped) {
            LOGD_AEIS("Get pending result id %d SKIPPED ...", buf->FrameId);
