...
XCamReturn
rk_aiq_adegamma_OverWriteCalibByCurAttr
(
...
)
{
    ...
    if(degamma_handle->adegammaAttr.mode < RK_AIQ_DEGAMMA_MODE_TOOL)
        memcpy(adegamma, degamma_handle->pCalibDb, sizeof(CalibDb_Degamma_t));
    else if(degamma_handle->adegammaAttr.mode == RK_AIQ_DEGAMMA_MODE_TOOL)
        memcpy(adegamma, &degamma_handle->adegammaAttr.stTool, sizeof(CalibDb_Degamma_t));
