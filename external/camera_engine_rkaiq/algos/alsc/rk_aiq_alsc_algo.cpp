...
XCamReturn applySensorLscOTP(const Cam17x17UShortMatrix_t *matrix, AlscOtpInfo_t *otpInfo)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    if (!matrix || !otpInfo || !otpInfo->flag || \
        !otpInfo->lsc_r || !otpInfo->lsc_b || \
        !otpInfo->lsc_gr || !otpInfo->lsc_gb)
        return XCAM_RETURN_BYPASS;

    Cam17x17UShortMatrix_t *pMatrix = const_cast<Cam17x17UShortMatrix_t*>(matrix);
...
static XCamReturn UpdateLscCalibPara(alsc_handle_t  hAlsc)
{
    ...
    if(!config_calib && hAlsc->eState != ALSC_STATE_INITIALIZED)
    {
        return(ret);
    }
        ...
        for (int k = 0; k < calib_lsc->aLscCof.lscResNum; k++) {
            ...
            for(int i = 0; i < calib_lsc->aLscCof.illuNum[c]; i++) {
                ...
                for (int j = 0; j < calib_lsc->aLscCof.illAll[c][i].tableUsedNO; j++) {
                   ...
                   applySensorLscOTP(pLscTableProfile->LscMatrix, &hAlsc->alscSwInfo.otpInfo);
