...
void getLscParameter(uint16_t *r, uint16_t *gr, uint16_t *gb, uint16_t *b, uint16_t *table, int width, int height, int bayer)
{
	...
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			...
			switch (bayerIdx)
			{
			case 0: table[idx] = r[idx]; break;
			case 1: table[idx] = gr[idx]; break;
			case 2: table[idx] = gb[idx]; break;
			case 3: table[idx] = b[idx]; break;
			default:
				break;
			}
		}
...
void separateBayerChannel(uint16_t* src, uint16_t* disR, uint16_t* disGr, uint16_t* disGb, uint16_t* disB, int width, int height, int bayer)
{
	...
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			...
			switch (bayerIdx)
			{
			case 0: *(disR++) = *(src + index); break;
			case 1: *(disGr++) = *(src + index); break;
			case 2: *(disGb++) = *(src + index); break;
			case 3: *(disB++) = *(src + index); break;
			default:
				break;
			}
		}
...
int getBayerIndex(int pattern, int row, int col)
{
	...
	if (pattern == BAYER_BGGR)
	{
		switch (tmp)
		{
		case 0: index = B_INDEX;  break;
		case 1: index = GB_INDEX; break;
		case 2: index = GR_INDEX; break;
		case 3: index = R_INDEX;  break;
		}
	}
	else if (pattern == BAYER_GBRG)
	{
		switch (tmp)
		{
		case 0: index = GB_INDEX; break;
		case 1: index = B_INDEX;  break;
		case 2: index = R_INDEX;  break;
		case 3: index = GR_INDEX; break;
		}
	}
	else if (pattern == BAYER_GRBG)
	{
		switch (tmp)
		{
		case 0: index = GR_INDEX; break;
		case 1: index = R_INDEX;  break;
		case 2: index = B_INDEX;  break;
		case 3: index = GB_INDEX; break;
		}
	}
	else if (pattern == BAYER_RGGB)
	{
		switch (tmp)
		{
		case 0: index = R_INDEX;  break;
		case 1: index = GR_INDEX; break;
		case 2: index = GB_INDEX; break;
		case 3: index = B_INDEX;  break;
		}
	}
