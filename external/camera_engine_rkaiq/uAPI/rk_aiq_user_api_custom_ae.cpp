                          ...
                          rk_aiq_customeAe_results_t* customAeProcRes)
{
    ...
    //vts here will cover the vts from rk_ae, for YuShi convenience
    if (pConfig->update_fps) {
        if ( (pConfig->fps_request <= 0) ) {
            //rkAeProcRes->new_ae_exp.frame_length_lines = pConfig->last_vts;
        } else {
            //if (pConfig->fps_request != pConfig->last_fps) {
            pConfig->vts_request = pConfig->PixelClockFreqMHZ * 1000000
                / (pConfig->PixelPeriodsPerLine * pConfig->fps_request);
            pConfig->last_vts = pConfig->vts_request;
            pConfig->last_fps = pConfig->fps_request;
            //}
        }
        pConfig->update_fps = false;
    }
    if(rkAeProcRes->new_ae_exp.LinearExp.exp_sensor_params.coarse_integration_time > pConfig->vts_request) {
        ...
        pConfig->last_vts = rkAeProcRes->new_ae_exp.frame_length_lines;
        pConfig->last_fps = pConfig->PixelClockFreqMHZ * 1000000 / (pConfig->last_vts * pConfig->PixelPeriodsPerLine);
    } else {
        rkAeProcRes->new_ae_exp.frame_length_lines = pConfig->vts_request;
        pConfig->last_vts = pConfig->vts_request;
        pConfig->last_fps = pConfig->fps_request;
    }
