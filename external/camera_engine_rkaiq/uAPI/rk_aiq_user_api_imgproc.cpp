...
XCamReturn rk_aiq_uapi_setInitExp(const rk_aiq_sys_ctx_t* ctx, float time, float gain)
{
    ...
    expSwAttr.stInitExp.stLinExpInitExp.InitTimeValue = time;
    expSwAttr.stInitExp.stLinExpInitExp.InitGainValue = gain;
...
XCamReturn
rk_aiq_uapi_setLut3dstrth(const rk_aiq_sys_ctx_t* ctx, float alpha)
{
    ...
    if (ctx == NULL) {
        ret = XCAM_RETURN_ERROR_PARAM;
        RKAIQ_IMGPROC_CHECK_RET(ret, "param error, set  3d Lut strength failed!");
    }
    if (alpha < 0.0) {
        alpha = 0;
        LOGW_A3DLUT("alpha is out of range, set default to 0!");
    }
    else if (alpha >1.0) {
        alpha = 1.0;
        LOGW_A3DLUT("alpha is out of range, set default to 1!");
    }

    ret = rk_aiq_user_api_a3dlut_GetAttrib(ctx, &attr);
    RKAIQ_IMGPROC_CHECK_RET(ret, "get 3d Lut strength failed!");
    attr.mode = RK_AIQ_LUT3D_MODE_AUTO;
    attr.stAuto.alpha = alpha;
    ret = rk_aiq_user_api_a3dlut_SetAttrib(ctx, attr);
    RKAIQ_IMGPROC_CHECK_RET(ret, "set 3d Lut strength failed!");
    IMGPROC_FUNC_EXIT
...
XCamReturn rk_aiq_uapi_getZoomCalibStat(const rk_aiq_sys_ctx_t* ctx, rk_aiq_af_zoomcalib_stat_t* stat)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    IMGPROC_FUNC_ENTER
    ret = rk_aiq_user_api_af_GetZoomCalibStat(ctx, stat);
    IMGPROC_FUNC_EXIT
...
XCamReturn rk_aiq_uapi_setDhzMode(const rk_aiq_sys_ctx_t* ctx, opMode_t mode)
{
    ...
    RKAIQ_IMGPROC_CHECK_RET(ret, "get attrib before setDhzMode auto failed!");
    attr.byPass = false;
    if (mode == OP_AUTO) {
...
XCamReturn rk_aiq_uapi_setMDhzStrth(const rk_aiq_sys_ctx_t* ctx, bool on, unsigned int level)
{
    ...
    if(attr.mode != RK_AIQ_DEHAZE_MODE_MANUAL) {
        ret = XCAM_RETURN_ERROR_PARAM;
        RKAIQ_IMGPROC_CHECK_RET(ret, "rk_aiq_uapi_SetDhzManualMode ERROR!");
    }
    attr.byPass = false;
    attr.mode = RK_AIQ_DEHAZE_MODE_MANUAL;
...
XCamReturn rk_aiq_uapi_enableDhz(const rk_aiq_sys_ctx_t* ctx)
{
    ...
    ret = rk_aiq_user_api_adehaze_getSwAttrib(ctx, &attr);
    RKAIQ_IMGPROC_CHECK_RET(ret, "get attrib before enable dehaze failed!");
    attr.byPass = false;
    attr.mode = RK_AIQ_DEHAZE_MODE_AUTO;
    ret = rk_aiq_user_api_adehaze_setSwAttrib(ctx, attr);
...
XCamReturn rk_aiq_uapi_setMEnhanceStrth(const rk_aiq_sys_ctx_t* ctx, bool on, unsigned int level)
{
    ...
    ret = rk_aiq_user_api_adehaze_getSwAttrib(ctx, &attr);
    if(attr.mode != RK_AIQ_ENHANCE_MODE_MANUAL) {
        ret = XCAM_RETURN_ERROR_PARAM;
        RKAIQ_IMGPROC_CHECK_RET(ret, "rk_aiq_uapi_SetDhzManualMode ERROR!");
    }
    attr.byPass = false;
    attr.mode = RK_AIQ_ENHANCE_MODE_MANUAL;
    attr.stManual.strength = level;
    ret = rk_aiq_user_api_adehaze_setSwAttrib(ctx, attr);
    RKAIQ_IMGPROC_CHECK_RET(ret, "setMDhzStrth failed!");
    IMGPROC_FUNC_EXIT
...
XCamReturn rk_aiq_uapi_SetDhzManualMode(const rk_aiq_sys_ctx_t* ctx, rk_aiq_dehaze_op_mode_t mode)
{
    ...
    ret = rk_aiq_user_api_adehaze_getSwAttrib(ctx, &attr);
    RKAIQ_IMGPROC_CHECK_RET(ret, "get attib before SetDhzManualMode failed!");
    attr.byPass = false;
    attr.mode = mode;
    ret = rk_aiq_user_api_adehaze_setSwAttrib(ctx, attr);
    RKAIQ_IMGPROC_CHECK_RET(ret, "SetDhzManualMode failed!");
    IMGPROC_FUNC_EXIT
    return ret;
}
...
XCamReturn rk_aiq_uapi_setGammaCoef(const rk_aiq_sys_ctx_t* ctx, float GammaCoef, float GammaOfst)
{
    ...
    if (GammaCoef <= 0 || GammaCoef > 10) {
        ret = XCAM_RETURN_ERROR_OUTOFRANGE;
        RKAIQ_IMGPROC_CHECK_RET(ret, "Gammacoef is out of range, setMHDRStrth failed!");
    }
    if (GammaOfst < -0.05 || GammaOfst > 0.05) {
        ret = XCAM_RETURN_ERROR_OUTOFRANGE;
        RKAIQ_IMGPROC_CHECK_RET(ret, "GammaOfst is out of range, setMHDRStrth failed!");
    }

    attr.mode = RK_AIQ_GAMMA_MODE_MANUAL;
    attr.stManual.en = true;
    attr.stManual.CurveType = RK_GAMMA_CURVE_TYPE_USER_DEFINE1;
    attr.stManual.user1.coef1 = GammaCoef;
    attr.stManual.user1.coef2 = GammaOfst;
    ret = rk_aiq_user_api_agamma_SetAttrib(ctx, attr);
    IMGPROC_FUNC_EXIT
    return ret;
}
...
XCamReturn rk_aiq_uapi_getGammaCoef(const rk_aiq_sys_ctx_t* ctx, float* GammaCoef, float* GammaOfst)
{
    ...
    ret = rk_aiq_user_api_agamma_GetAttrib(ctx, &attr);
    if(attr.mode == RK_AIQ_GAMMA_MODE_MANUAL && attr.stManual.en && attr.stManual.CurveType == RK_GAMMA_CURVE_TYPE_USER_DEFINE1) {
        *GammaCoef = attr.stManual.user1.coef1;
        *GammaOfst = attr.stManual.user1.coef2;
    }
    else {
        LOGE("Not in manual USER_DEFINE1 mode!");
        *GammaCoef = 0;
        *GammaOfst = 0;
    }
    IMGPROC_FUNC_EXIT
...
XCamReturn rk_aiq_uapi_setGammaCurve(const rk_aiq_sys_ctx_t* ctx, int* GammaTable)
{
    ...
    for(int i = 0; i < 45; i++)
        if (GammaTable[i] < 0 || GammaTable[i] > 4095) {
            ret = XCAM_RETURN_ERROR_OUTOFRANGE;
            RKAIQ_IMGPROC_CHECK_RET(ret, "GammaTable is out of range, setMHDRStrth failed!");
        }

    attr.mode = RK_AIQ_GAMMA_MODE_MANUAL;
    attr.stManual.en = true;
    attr.stManual.CurveType = RK_GAMMA_CURVE_TYPE_USER_DEFINE2;
    for(int i = 0; i < 45; i++)
        attr.stManual.user2.gamma_table[i] = GammaTable[i];
    attr.stManual.user2.gamma_out_offset = 0;
    attr.stManual.user2.gamma_out_segnum = 0;
...
XCamReturn rk_aiq_uapi_getGammaCurve(const rk_aiq_sys_ctx_t* ctx, int* GammaTable)
{
    ...
    ret = rk_aiq_user_api_agamma_GetAttrib(ctx, &attr);
    if(attr.mode == RK_AIQ_GAMMA_MODE_MANUAL && attr.stManual.en && attr.stManual.CurveType == RK_GAMMA_CURVE_TYPE_USER_DEFINE2)
        for(int i = 0; i < 45; i++)
            GammaTable[i] = attr.stManual.user2.gamma_table[i];
    else {
        LOGE("Not in manual USER_DEFINE2 mode!");
        for(int i = 0; i < 45; i++)
            GammaTable[i] = 0;
    }
    IMGPROC_FUNC_EXIT
...
extern "C" {
    ...
    void rk_aiq_uapi_customAE_setFrameRate(RkAiqAlgoContext* context, float fps);
    float rk_aiq_uapi_customAE_getFrameRate(RkAiqAlgoContext* context);
...
XCamReturn rk_aiq_uapi_setFrameRate(const rk_aiq_sys_ctx_t* ctx, frameRateInfo_t info)
{
    ...
    if (ctx->_analyzer->mCurCustomAeAlgoHdl.ptr() != NULL) {
        rk_aiq_uapi_customAE_setFrameRate(ctx->_analyzer->mCurCustomAeAlgoHdl->getAlgoCtx(), info.fps);
    }
...
XCamReturn rk_aiq_uapi_getFrameRate(const rk_aiq_sys_ctx_t* ctx, frameRateInfo_t *info)
{
    ...
    if (!isHDRmode(ctx) && ctx->_analyzer->mCurCustomAeAlgoHdl.ptr() != NULL) {
        info->fps = rk_aiq_uapi_customAE_getFrameRate(ctx->_analyzer->mCurCustomAeAlgoHdl->getAlgoCtx());
    }
