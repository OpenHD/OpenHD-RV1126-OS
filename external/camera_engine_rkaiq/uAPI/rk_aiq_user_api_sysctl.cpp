...
rk_aiq_uapi_sysctl_setReadBackMode(rk_aiq_sys_ctx_t* ctx, bool on)
{
    ...
    ret = ctx->_rkAiqManager->set_read_back_mode(on);
    EXIT_XCORE_FUNCTION();
...
#ifdef __linux__
    snprintf(lock_path, 255 - 15, "/tmp/aiq-%s.lock", ctx->_sensor_entity_name);
    ctx->_lock_file = fopen(lock_path, "w+");
    auto lock_res   = flock(fileno(ctx->_lock_file), LOCK_EX);
    if (!lock_res) {
...
void rk_aiq_uapi_setRawBufNum(rk_aiq_sys_ctx_t* ctx, uint16_t buf_num)
{
    ENTER_XCORE_FUNCTION();
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    ret = ctx->_rkAiqManager->setTxBufNum(buf_num);
    EXIT_XCORE_FUNCTION();
}
...
XCamReturn rk_aiq_uapi_sysctl_enumStaticMetasByPhyId(
    ...
    if (!static_info) return XCAM_RETURN_ERROR_FAILED;

#ifdef RK_SIMULATOR_HW
    /* nothing to do now*/
    static_info = NULL;
#else
    rk_aiq_static_info_t* tmp =
        CamHwIsp20::getStaticCamHwInfoByPhyId(NULL, index);
    if (tmp)
...
rk_aiq_uapi_sysctl_getRkRawFrameInfo(const rk_aiq_sys_ctx_t* ctx, rkraw_frame_info_t *frame_info)
{
    ENTER_XCORE_FUNCTION();
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    ret = ctx->_rkAiqManager->get_rkraw_frame_info(frame_info);
    EXIT_XCORE_FUNCTION();

    return ret;
}

XCamReturn rk_aiq_uapi_getFrameInfo_fromRkRaw
(
    void* rawdata,
    ...
    parser.parse_rk_rawdata(rawdata);
    memcpy(frame_info, parser.get_frame_info(), sizeof(rkraw_frame_info_t));
...
extern XCamReturn
rk_aiq_ahdr_OverWriteCalibByCurAttr(const RkAiqAlgoContext *ctx,
                                    CalibDb_Ahdr_Para_t *ahdr);
static XCamReturn
_ahdr_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_Ahdr_Para_t *calibDb_ahdr)
{
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_ahdr_OverWriteCalibByCurAttr(ctx, calibDb_ahdr);
        }
    }
    return(ret);
}
...
extern XCamReturn
rk_aiq_3dlut_OverWriteCalibByCurAttr(const RkAiqAlgoContext *ctx,
                                     CalibDb_Lut3d_t *lut3d);
static XCamReturn
_3dlut_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_Lut3d_t *lut3d)
{
    XCamReturn ret = XCAM_RETURN_ERROR_FAILED;
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_3dlut_OverWriteCalibByCurAttr(ctx, lut3d);
        }
    }
    return(ret);
...
extern XCamReturn
rk_aiq_ccm_OverWriteCalibByCurAttr(const RkAiqAlgoContext *ctx,
                                   CalibDb_Ccm_t *ccm);
static XCamReturn
_ccm_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_Ccm_t *ccm)
{
    XCamReturn ret = XCAM_RETURN_ERROR_FAILED;
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_ccm_OverWriteCalibByCurAttr(ctx, ccm);
        }
    }
    return(ret);
...
static void
_overide_calib_cproc(CalibDb_cProc_t* calib_cproc, acp_attrib_t* acp_attrib)
{
    calib_cproc->brightness = acp_attrib->brightness;
    calib_cproc->contrast = acp_attrib->contrast;
    calib_cproc->saturation = acp_attrib->saturation;
    calib_cproc->hue = acp_attrib->hue;
}

extern XCamReturn
rk_aiq_awb_OverWriteCalibByCurAttr(const RkAiqAlgoContext *ctx,
                                   CalibDb_Awb_Para_t *awb);

static XCamReturn
_awb_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_Awb_Para_t *calibDb_awb)

{
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_awb_OverWriteCalibByCurAttr(ctx, calibDb_awb);
        }
    }
    return(ret);
...
extern XCamReturn
rk_aiq_ae_OverWriteCalibByCurAttr(const RkAiqAlgoContext *ctx,
                                  CalibDb_Aec_Para_t *ae);

static XCamReturn
_ae_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_Aec_Para_t *calibDb_ae)

{
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_ae_OverWriteCalibByCurAttr(ctx, calibDb_ae);
        }
    }
    return(ret);
...
extern XCamReturn
rk_aiq_af_OverWriteCalibByCurAttr(const RkAiqAlgoContext *ctx,
                                  CalibDb_AF_t *af);
static XCamReturn
_af_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_AF_t *calibDb_af)

{
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_af_OverWriteCalibByCurAttr(ctx, calibDb_af);
        }
    }
    return(ret);
...
static XCamReturn
_bayernr_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_BayerNr_2_t *pCalibdb)

{
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_bayernr_OverWriteCalibByCurAttr(ctx, pCalibdb);
        }
    }
    return(ret);
...
static XCamReturn
_ynr_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_YNR_2_t* pCalibdb)

{
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_ynr_OverWriteCalibByCurAttr(ctx, pCalibdb);
        }
    }
    return(ret);
...
static XCamReturn
_uvnr_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_UVNR_2_t* pCalibdb)

{
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_uvnr_OverWriteCalibByCurAttr(ctx, pCalibdb);
        }
    }
    return(ret);
...
static XCamReturn
_mfnr_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_MFNR_2_t* pCalibdb)

{
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_mfnr_OverWriteCalibByCurAttr(ctx, pCalibdb);
        }
    }
    return(ret);
...
static XCamReturn
_sharp_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_Sharp_2_t* pCalibdb)

{
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_sharp_OverWriteCalibByCurAttr(ctx, pCalibdb);
        }
    }
    return(ret);
...
static XCamReturn
_edgefilter_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_EdgeFilter_2_t* pCalibdb)

{
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_edgefilter_OverWriteCalibByCurAttr(ctx, pCalibdb);
        }
    }
    return(ret);
...
static XCamReturn
_ablc_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_Blc_t* pCalibdb)

{
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_ablc_OverWriteCalibByCurAttr(ctx, pCalibdb);
        }
    }
    return(ret);
...
extern XCamReturn
rk_aiq_adehaze_OverWriteCalibByCurAttr(const RkAiqAlgoContext *ctx,
                                       CalibDb_Dehaze_t *adehaze);
static XCamReturn
_adehaze_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_Dehaze_t *calibDb_adehaze)
{
    XCamReturn ret = XCAM_RETURN_ERROR_FAILED;
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_adehaze_OverWriteCalibByCurAttr(ctx, calibDb_adehaze);
        }
    }
    return(ret);
}
...
extern XCamReturn
rk_aiq_agamma_OverWriteCalibByCurAttr(const RkAiqAlgoContext *ctx,
                                      CalibDb_Gamma_t *agamma);
static XCamReturn
_agamma_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_Gamma_t *calibDb_agamma)
{
    XCamReturn ret = XCAM_RETURN_ERROR_FAILED;
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_agamma_OverWriteCalibByCurAttr(ctx, calibDb_agamma);
        }
    }
    return(ret);
}
...
extern XCamReturn
rk_aiq_adegamma_OverWriteCalibByCurAttr(const RkAiqAlgoContext *ctx,
                                        CalibDb_Degamma_t *adegamma);
static XCamReturn
_adegamma_OverWriteCalibByCurAttr(const rk_aiq_sys_ctx_t* sys_ctx, CalibDb_Degamma_t *calibDb_adegamma)
{
    XCamReturn ret = XCAM_RETURN_ERROR_FAILED;
    ...
    if (algo_handle) {
        RkAiqAlgoContext *ctx = algo_handle->getAlgoCtx();
        if(ctx) {
            ret = rk_aiq_adegamma_OverWriteCalibByCurAttr(ctx, calibDb_adegamma);
        }
    }
    return(ret);
}
...
rk_aiq_uapi_sysctl_getCurCalib(const rk_aiq_sys_ctx_t* ctx)
{
    ...
    // TODO: get all current module params,
    // should not be called with RKAIQ_API_SMART_LOCK
    acp_attrib_t acp_attrib;
    ret = rk_aiq_user_api_acp_GetAttrib(ctx, &acp_attrib);
    ret = _awb_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->awb);
    ret = _ae_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->aec);
    ret = _af_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->af);

    ret = _bayernr_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->bayerNr);
    ret = _mfnr_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->mfnr);
    ret = _ynr_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->ynr);
    ret = _uvnr_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->uvnr);
    ret = _sharp_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->sharp);
    ret = _edgefilter_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->edgeFilter);
    ret = _ablc_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->blc);

    ret = _3dlut_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->lut3d);
    ret = _ccm_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->ccm);
    ret = _ahdr_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->ahdr);
    ret = _adehaze_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->dehaze);
    ret = _agamma_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->gamma);
    ret = _adegamma_OverWriteCalibByCurAttr(ctx, &ctx->_calibDb->degamma);

    // Lock
    RKAIQ_API_SMART_LOCK(ctx);

    // overide calib with current params
    _overide_calib_cproc(&ctx->_calibDb->cProc, &acp_attrib);
