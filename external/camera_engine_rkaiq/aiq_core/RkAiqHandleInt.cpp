...
RkAiqHandleIntCom::waitSignal()
{
    ...
    if (mAiqCore->isRunningState()) {
        int32_t ret_b = mUpdateCond.timedwait(mCfgMutex, 300000);
        if (ret_b) {
            LOGE_ANALYZER("mUpdateCond.timedwait error: %d", ret_b);
            updateAtt = false;
            if (ret_b == ETIMEDOUT)
...
RkAiqAeHandleInt::updateConfig(bool needSync)
{
    ...
    if (updateLinExpAttr) {
        ...
        updateAeAttr |= UPDATE_LINEXPATTR;
    ...
    if (updateHdrExpAttr) {
        ...
        updateAeAttr |= UPDATE_HDREXPATTR;
    ...
    if (updateLinAeDayRouteAttr) {
        ...
        updateAeAttr |= UPDATE_LINAEROUTEATTR;
    ...
    if (updateHdrAeDayRouteAttr) {
        ...
        updateAeAttr |= UPDATE_HDRAEROUTEATTR;
    ...
    if (updateExpWinAttr) {
        ...
        updateAeAttr |= UPDATE_EXPWINATTR;
    ...
    pAecCfg->IsReconfig |= updateAeAttr;
    updateAeAttr = 0;
    if (needSync)
        mCfgMutex.unlock();
...
RkAiqAeHandleInt::setLinAeDayRouteAttr(Uapi_LinAeRouteAttr_t LinAeDayRouteAttr)
{
    ...
    if (0 != memcmp(&mCurLinAeDayRouteAttr, &LinAeDayRouteAttr, sizeof(Uapi_LinAeRouteAttr_t))) {
        ...
        ret = waitSignal();
        if (ret != XCAM_RETURN_NO_ERROR) {
            mNewLinAeDayRouteAttr = mCurLinAeDayRouteAttr;
            updateLinAeDayRouteAttr = false;
        }
    }
    mCfgMutex.unlock();
...
RkAiqAeHandleInt::setHdrAeDayRouteAttr(Uapi_HdrAeRouteAttr_t HdrAeDayRouteAttr)
{
    ...
    if (0 != memcmp(&mCurHdrAeDayRouteAttr, &HdrAeDayRouteAttr, sizeof(Uapi_HdrAeRouteAttr_t))) {
        ...
        ret = waitSignal();
        if (ret != XCAM_RETURN_NO_ERROR) {
            mNewHdrAeDayRouteAttr = mCurHdrAeDayRouteAttr;
            updateHdrAeDayRouteAttr = false;
        }
    }
    mCfgMutex.unlock();
...
RkAiqAeHandleInt::setLinAeNightRouteAttr(Uapi_LinAeRouteAttr_t LinAeNightRouteAttr)
{
    ...
    if (0 != memcmp(&mCurLinAeNightRouteAttr, &LinAeNightRouteAttr, sizeof(Uapi_LinAeRouteAttr_t))) {
        ...
        ret = waitSignal();
        if (ret != XCAM_RETURN_NO_ERROR) {
            mNewLinAeNightRouteAttr = mCurLinAeNightRouteAttr;
            updateLinAeNightRouteAttr = false;
        }
    }
    mCfgMutex.unlock();
...
RkAiqAeHandleInt::setHdrAeNightRouteAttr(Uapi_HdrAeRouteAttr_t HdrAeNightRouteAttr)
{
    ...
    if (0 != memcmp(&mCurHdrAeNightRouteAttr, &HdrAeNightRouteAttr, sizeof(Uapi_HdrAeRouteAttr_t))) {
        ...
        ret = waitSignal();
        if (ret != XCAM_RETURN_NO_ERROR) {
            mNewHdrAeNightRouteAttr = mCurHdrAeNightRouteAttr;
            updateHdrAeNightRouteAttr = false;
        }
    }
    mCfgMutex.unlock();
...
RkAiqAwbHandleInt::prepare()
{
    ...
    int lightNum = shared->calib->awb.measure_para_v200.lightNum;
    if (lightNum <= 7 && shared->isp_online) {
        awb_config_int->needTimeShareMode = false;
    } else if (lightNum > 7 && shared->isp_online) {
        ret = XCAM_RETURN_ERROR_PARAM;
        RKAIQCORE_CHECK_RET(ret, "Not suppport awb lightNum:%d > 7 with isp online", lightNum);
    } else {
        awb_config_int->needTimeShareMode = true;
    }
...
RkAiqAwbHandleInt::preProcess()
{
    ...
    RkAiqIspStats* ispStats = &shared->ispStats;
// for otp awb
    awb_pre_int->awb_otp = shared->snsDes.otp_awb;
...
XCamReturn
RkAiqAfHandleInt::GetZoomCalibStat(rk_aiq_af_zoomcalib_stat_t* stat)
{
    ENTER_ANALYZER_FUNCTION();

    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    rk_aiq_uapi_af_getZoomCalibState(mAlgoCtx, stat);
...
RkAiqAnrHandleInt::updateConfig(bool needSync)
{
    ...
    if(UpdateMfnrIQpara) {
        mCurMfnrIQpara = mNewMfnrIQpara;
        UpdateMfnrIQpara = false;
        rk_aiq_uapi_anr_SetMfnrIQPara(mAlgoCtx, &mCurMfnrIQpara, false);
        sendSignal();
    }
...
XCamReturn
RkAiqAnrHandleInt::setMfnrIQPara(rk_aiq_mfnr_IQPara_t *para)
{
    ENTER_ANALYZER_FUNCTION();
    ...
    // if something changed
    if (0 != memcmp(&mCurMfnrIQpara, para, sizeof(rk_aiq_mfnr_IQPara_t))) {
        RkAiqCore::RkAiqAlgosShared_t* shared = &mAiqCore->mAlogsSharedParams;
        if (shared->calib->mfnr.enable && shared->calib->mfnr.motion_detect_en) {
            if( !para->stMfnrPara.enable) {
                para->stMfnrPara.enable = !para->stMfnrPara.enable;
                LOGE("motion detect is running, disable mfnr is not permit!");
            }
        }
        mNewMfnrIQpara = *para;
        UpdateMfnrIQpara = true;
        ret = waitSignal();
        if (ret != XCAM_RETURN_NO_ERROR) {
            mNewMfnrIQpara = mCurMfnrIQpara;
            UpdateMfnrIQpara = false;
        }
    }
EXIT:
...
XCamReturn
RkAiqAnrHandleInt::getMfnrIQPara(rk_aiq_mfnr_IQPara_t *para)
{
    ENTER_ANALYZER_FUNCTION();

    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    ret = rk_aiq_uapi_anr_GetMfnrIQPara(mAlgoCtx, para);
...
RkAiqAlscHandleInt::prepare()
{
    ...
    RkAiqCore::RkAiqAlgosShared_t* shared = &mAiqCore->mAlogsSharedParams;
    alsc_config_int->alsc_sw_info.bayerPattern= getBayerPattern(shared->snsDes.sensor_pixelformat);
    alsc_config_int->alsc_sw_info.ispAcqWidth = shared->snsDes.isp_acq_width;
    alsc_config_int->alsc_sw_info.ispAcqHeight = shared->snsDes.isp_acq_height;
    if (shared->snsDes.otp_lsc && shared->snsDes.otp_lsc->flag) {
        alsc_config_int->alsc_sw_info.otpInfo.flag = shared->snsDes.otp_lsc->flag;
        alsc_config_int->alsc_sw_info.otpInfo.width = shared->snsDes.otp_lsc->width;
        alsc_config_int->alsc_sw_info.otpInfo.height = shared->snsDes.otp_lsc->height;
        alsc_config_int->alsc_sw_info.otpInfo.table_size = shared->snsDes.otp_lsc->table_size;
        alsc_config_int->alsc_sw_info.otpInfo.lsc_r = shared->snsDes.otp_lsc->lsc_r;
        alsc_config_int->alsc_sw_info.otpInfo.lsc_b = shared->snsDes.otp_lsc->lsc_b;
        alsc_config_int->alsc_sw_info.otpInfo.lsc_gr = shared->snsDes.otp_lsc->lsc_gr;
        alsc_config_int->alsc_sw_info.otpInfo.lsc_gb = shared->snsDes.otp_lsc->lsc_gb;
    } else {
...
RkAiqAeisHandleInt::prepare()
{
    ...
    aeis_config_int->common.mems_sensor_intf = mAiqCore->getMemsSensorIntf();
...
RkAiqAeisHandleInt::processing()
{
    ...
    RkAiqProcResComb* comb = &shared->procResComb;
    RkAiqIspStats* ispStats = &shared->ispStats;
    rk_aiq_eis_input_info* eisInput = &shared->eis_input_info;

    aeis_proc_int->common.sof = eisInput->timestamp;
    ...
    aeis_proc_int->common.rolling_shutter_skew =
        shared->eis_input_info.exp_Info.line_length_pixels *
        nano_seconds_per_second /
        shared->eis_input_info.exp_Info.pixel_clock_freq_mhz *
        shared->snsDes.isp_acq_height;
    aeis_proc_int->common.rolling_shutter_skew *=
        shared->snsDes.sensor_readout_linecnt_perline > 0
            ? shared->snsDes.sensor_readout_linecnt_perline
            : 1;
    aeis_proc_int->common.rolling_shutter_skew /= 1000000ULL;

    // TODO(Cody): The exposure time is identical to integration time
    aeis_proc_int->common.integration_time =
        shared->eis_input_info.exp_Info.LinearExp.exp_real_params
            .integration_time;
    aeis_proc_int->common.analog_gain =
        shared->eis_input_info.exp_Info.LinearExp.exp_real_params.analog_gain;

    aeis_proc_int->orb_stats_valid = shared->ispStats.orb_stats_valid;
    aeis_proc_int->orb_stats = shared->ispStats.orb_stats;
    memcpy(&aeis_proc_int->nr_buf_info, &shared->eis_input_info.nr_buf_info, sizeof(aeis_proc_int->nr_buf_info));
    aeis_proc_int->common.frame_width = shared->eis_input_info.nr_buf_info.width;
    aeis_proc_int->common.frame_height = shared->eis_input_info.nr_buf_info.height;
    aeis_proc_int->common.frame_id = shared->eis_input_info.id;
...
XCamReturn
RkAiqAeisHandleInt::postProcess()
{
    ENTER_ANALYZER_FUNCTION();
...
XCamReturn
RkAiqAeisHandleInt::updateConfig(bool needSync)
{
    ENTER_ANALYZER_FUNCTION();
...
XCamReturn
RkAiqAeisHandleInt::setAttrib(rk_aiq_eis_attrib_t att)
{
    ENTER_ANALYZER_FUNCTION();
...
XCamReturn
RkAiqAeisHandleInt::getAttrib(rk_aiq_eis_attrib_t *att)
{
    ENTER_ANALYZER_FUNCTION();
