...
static RkAiqAlgoDesComm* g_default_3a_des[] = {
    ...
    (RkAiqAlgoDesComm*)&g_RkIspAlgoDescAeis,
};
...
bool
RkAiqCoreEisAnalyzerThread::loop()
{
    ENTER_ANALYZER_FUNCTION();

    const static int32_t timeout = -1;

    SmartPtr<RkAiqEisInputInfo> nrStatsBuf = mBufQueue.pop (timeout);
    if (!nrStatsBuf.ptr()) {
        LOGW_ANALYZER("RkAiqCoreEisAnalyzerThread got empty stats, stop thread");
        return false;
    }

    XCamReturn ret = mRkAiqCore->eisAnalyze(nrStatsBuf);
    if (ret == XCAM_RETURN_NO_ERROR || ret == XCAM_RETURN_BYPASS)
        return true;

    LOGE_ANALYZER("RkAiqCoreEisAnalyzerThread failed to analyze stats");
...
RkAiqCore::RkAiqCore()
    ...
    , mRkAiqCoreEisAnalyzerTh(new RkAiqCoreEisAnalyzerThread(this))
    ...
    , mAiqTnrParamsPool(new RkAiqTnrParamsPool("RkAiqTnrParamsPool", RkAiqCore::DEFAULT_POOL_SIZE))
    , mAiqFecParamsPool(new RkAiqFecParamsPool("RkAiqFecParamsPool", RkAiqCore::DEFAULT_POOL_SIZE))
...
RkAiqCore::start()
{
    ...
    if (mRkAiqCoreEisAnalyzerTh.ptr()) {
        mRkAiqCoreEisAnalyzerTh->triger_start();
        mRkAiqCoreEisAnalyzerTh->start();
    }
...
RkAiqCore::stop()
{
    ...
    if (mRkAiqCoreEisAnalyzerTh.ptr()) {
        mRkAiqCoreEisAnalyzerTh->triger_stop();
        mRkAiqCoreEisAnalyzerTh->stop();
    }
                   ...
                   int mode)
{
    ...
    // split isp working mode and sensor working mode
    mAlogsSharedParams.isp_online = mode & RK_AIQ_WORKING_MODE_ISP_ONLINE ? true : false;
    mode &= ~RK_AIQ_WORKING_MODE_ISP_ONLINE;
    ...
    mAlogsSharedParams.eis_input_info.sensor_readout_linecnt_perline = \
        sensor_des->sensor_readout_linecnt_perline;
    mAlogsSharedParams.eis_input_info.nr_buf_info.width = sensor_des->isp_acq_width;
    mAlogsSharedParams.eis_input_info.nr_buf_info.height = sensor_des->isp_acq_height;
...
#define PREPARE_CUSTOM_ALGO(at) \
    ...
    if (mAiqCurParams->data()->mTnrParams.ptr()) {
        mAiqCurParams->data()->mTnrParams->data()->frame_id = 0;
    }

    if (mAiqCurParams->data()->mFecParams.ptr()) {
        mAiqCurParams->data()->mFecParams->data()->frame_id = 0;
    }
...
RkAiqCore::analyzeInternal(enum rk_aiq_core_analyze_type_e type)
{
    ...
    if (type == RK_AIQ_CORE_ANALYZE_ALL || \
        ...
        if (mAiqTnrParamsPool->has_free_items()) {
            aiqParams->mTnrParams = mAiqTnrParamsPool->get_item();
            aiqParams->mTnrParams->data()->update_mask = 0;
        } else {
            LOGE_ANALYZER("no free tnr params buffer!");
            return NULL;
        }
    }
#endif
...
RkAiqCore::genIspResult(RkAiqFullParams *aiqParams, enum rk_aiq_core_analyze_type_e type)
{
    ...
    if (type == RK_AIQ_CORE_ANALYZE_ALL || \
        ...
        mAiqCurParams->data()->mTnrParams = aiqParams->mTnrParams;
        // mAiqCurParams->data()->mFecParams = aiqParams->mFecParams;
    }

    if (type == RK_AIQ_CORE_ANALYZE_ALL || \
        type == RK_AIQ_CORE_ANALYZE_AEIS) {
        //genIspAeisResult(aiqParams);
        //mAiqCurParams->data()->mFecParams = aiqParams->mFecParams;
    }
#endif
...
RkAiqCore::genIspAhdrResult(RkAiqFullParams* params)
{
    ...
    if (algo_id == 0) {
        ...
        isp_param->ahdr_proc_res.TmoFlicker.AeConvergedPdf =
            ahdr_rk->AhdrProcRes.TmoFlicker.AeConvergedPdf;
        isp_param->ahdr_proc_res.TmoFlicker.IsAeConverged =
            ahdr_rk->AhdrProcRes.TmoFlicker.IsAeConverged;
        isp_param->ahdr_proc_res.TmoFlicker.ro_hdrtmo_lgmean =
            ahdr_rk->AhdrProcRes.TmoFlicker.ro_hdrtmo_lgmean;
        isp_param->ahdr_proc_res.TmoFlicker.damp =
            ahdr_rk->AhdrProcRes.TmoFlicker.damp;
...
RkAiqCore::genIspAnrResult(RkAiqFullParams* params)
{
    ...
    if (algo_id == 0) {
        ...
        SmartPtr<rk_aiq_tnr_params_t> tnr_param =
            params->mTnrParams->data();
...
RkAiqCore::genIspAfecResult(RkAiqFullParams* params)
{
    ...
    SmartPtr<rk_aiq_ispp_meas_params_t> ispp_param =
        params->mIsppMeasParams->data();
#else
    SmartPtr<rk_aiq_fec_params_t> fec_params =
        params->mFecParams->data();
#endif
    ...
    if (fec_params->fec.usage == ISPP_MODULE_FEC_ST) {
    ...
    if (algo_id == 0) {
        ...
        if (afec_rk->afec_result.update) {
            ...
            fec_params->fec.fec_en = afec_rk->afec_result.sw_fec_en;
            fec_params->fec.usage = ISPP_MODULE_FEC;
            if (fec_params->fec.fec_en) {
                fec_params->fec.config = true;
                fec_params->fec.crop_en = afec_rk->afec_result.crop_en;
                fec_params->fec.crop_width = afec_rk->afec_result.crop_width;
                fec_params->fec.crop_height = afec_rk->afec_result.crop_height;
                fec_params->fec.mesh_density = afec_rk->afec_result.mesh_density;
                fec_params->fec.mesh_size = afec_rk->afec_result.mesh_size;
                fec_params->fec.mesh_buf_fd = afec_rk->afec_result.mesh_buf_fd;
                //memcpy(fec_params->fec.sw_mesh_xi, afec_rk->afec_result.meshxi, sizeof(fec_params->fec.sw_mesh_xi));
                //memcpy(fec_params->fec.sw_mesh_xf, afec_rk->afec_result.meshxf, sizeof(fec_params->fec.sw_mesh_xf));
                //memcpy(fec_params->fec.sw_mesh_yi, afec_rk->afec_result.meshyi, sizeof(fec_params->fec.sw_mesh_yi));
                //memcpy(fec_params->fec.sw_mesh_yf, afec_rk->afec_result.meshyf, sizeof(fec_params->fec.sw_mesh_yf));
            }
...
XCamReturn
RkAiqCore::genIspAfecResult(RkAiqFullParams* params, RkAiqAlgoResCom* outparams)
{
    ENTER_ANALYZER_FUNCTION();
    ...
    // gen rk afec result
    if (algo_id == 0) {
        RkAiqAlgoProcResAfecInt* afec_rk = (RkAiqAlgoProcResAfecInt*) outparams;
        ...
        if (afec_rk->afec_result.update) {
            ...
            LOGE_ANALYZER("afec update %d, usage: 0x%x", afec_rk->afec_result.update, fec_params->fec.usage);
        } else
            fec_params->update_mask &= ~RKAIQ_ISPP_FEC_ID;
    }
...
RkAiqCore::genIspAeisResult(RkAiqFullParams* params)
{
    ...
    RkAiqAlgoProcResAeis* aeis_com =
        mAlogsSharedParams.procResComb.aeis_proc_res;
#ifdef RK_SIMULATOR_HW
    SmartPtr<rk_aiq_ispp_meas_params_t> ispp_param =
        params->mIsppMeasParams->data();
#else
    ...
    if (algo_id == 0) {
        ...
        if (aeis_rk->update) {
            ...
            fec_params->fec.fec_en = aeis_rk->fec_en;
            fec_params->fec.usage = ISPP_MODULE_FEC_ST;
            if (fec_params->fec.fec_en) {
                if (aeis_rk->fd >= 0) {
                    fec_params->fec.config = true;
                    fec_params->fec.mesh_density = aeis_rk->mesh_density;
                    fec_params->fec.mesh_size = aeis_rk->mesh_size;
                    fec_params->fec.mesh_buf_fd = aeis_rk->fd;
                    fec_params->fec.img_buf_index = aeis_rk->img_buf_index;
                    fec_params->fec.img_buf_size = aeis_rk->img_size;
                    fec_params->fec.frame_id = aeis_rk->frame_id;
                    fec_params->frame_id = aeis_rk->frame_id;
                } else {
                    fec_params->fec.config = false;
                }
            } else {
                fec_params->update_mask &= ~ISPP_MODULE_FEC_ST;
            }
        }
        LOGD_ANALYZER("eis update %d, fec: en %d, config %d, fd %d, idx %d",
                      aeis_rk->update, fec_params->fec.fec_en,
                      fec_params->fec.config, fec_params->fec.mesh_buf_fd,
                      fec_params->fec.img_buf_index);
    }
...
XCamReturn
RkAiqCore::genIspAeisResult(RkAiqFullParams* params, RkAiqAlgoResCom* outparams)
{
    ENTER_ANALYZER_FUNCTION();
    ...
    int algo_id = (*handle)->getAlgoId();
    if (algo_id == 0) {
        RkAiqAlgoProcResAeisInt* aeis_rk = (RkAiqAlgoProcResAeisInt*)outparams;
        if (aeis_rk->update) {
            fec_params->update_mask |= ISPP_MODULE_FEC_ST;
            fec_params->fec.fec_en = aeis_rk->fec_en;
            fec_params->fec.usage = ISPP_MODULE_FEC_ST;
...
XCamReturn
RkAiqCore::pushEisInputInfo (SmartPtr<VideoBuffer> &buffer,
                             ...
                             uint64_t timestamp)
{
    ...
    XCAM_ASSERT(buffer.ptr ());
    SmartPtr<RkAiqEisInputInfo> eisInputInfo    = new RkAiqEisInputInfo ();
    if (expParams.ptr())
        eisInputInfo->expParams                 = expParams;
    eisInputInfo->nrStatsBuf                    = buffer;
    eisInputInfo->timestamp                     = timestamp;
    if (mRkAiqCoreEisAnalyzerTh.ptr ())
        mRkAiqCoreEisAnalyzerTh->push_buf (eisInputInfo);
...
#define ADD_ALGO_HANDLE(lc, BC) \
    ...
    if (RK_AIQ_ALGO_TYPE_##BC == RK_AIQ_ALGO_TYPE_AEIS || \
        RK_AIQ_ALGO_TYPE_##BC == RK_AIQ_ALGO_TYPE_AFEC) { \
        Callback<int32_t(RkAiqAlgoType_t, RkAiqAlgoResCom*)>::func = \
            std::bind(&RkAiqCore::OnAlgosResCallback, this, std::placeholders::_1, std::placeholders::_2); \
        mAlogsSharedParams.ctxCfigs[RK_AIQ_ALGO_TYPE_##BC].resCallback = static_cast<AlgosResultCallback>( \
            Callback<int32_t(RkAiqAlgoType_t, RkAiqAlgoResCom*)>::callback); \
    } \
    ...
    enableAlgo(RK_AIQ_ALGO_TYPE_AEIS, 0, true);
#endif
#endif
}
...
RkAiqCore::analyze(const SmartPtr<VideoBuffer> &buffer)
{
    ...
    if (fullParam.ptr() && mCb) {
        ...
        // fullParam->data()->mTnrParams->data()->frame_id = buffer->get_sequence() + 1;
        // fullParam->data()->mFecParams->data()->frame_id = buffer->get_sequence() + 1;
        /*
...
RkAiqCore::events_analyze(const SmartPtr<ispHwEvt_t> &evts)
{
    ...
    if (fullParam.ptr() && mCb) {
        ...
        fullParam->data()->mTnrParams->data()->frame_id = frameId;
        /* TODO: Compatible with FEC and EIS  */
        // fullParam->data()->mFecParams->data()->frame_id = frameId;
        mCb->rkAiqCalcDone(fullParam);
    }
...
XCamReturn
RkAiqCore::eisAnalyze(const SmartPtr<RkAiqEisInputInfo_s> &eisInputInfo)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    SmartPtr<RkAiqFullParamsProxy> fullParam;

    const SmartPtr<V4l2BufferProxy> nrStatsBufProxy =
        eisInputInfo->nrStatsBuf.dynamic_cast_ptr<V4l2BufferProxy>();
    rk_aiq_eis_input_info *eis_input_info   = &mAlogsSharedParams.eis_input_info;
    static RKAiqAecExpInfo_t last_exp = {0};

    eis_input_info->id                      = nrStatsBufProxy->get_sequence ();
    eis_input_info->timestamp               = eisInputInfo->timestamp;
    LOGD_ANALYZER("%s, id(%d), sof timestamp: %" PRId64 "ms",
            __func__,
            eis_input_info->id,
            eis_input_info->timestamp/1000000);

    if (eisInputInfo->expParams.ptr ()) {
        memcpy(&eis_input_info->exp_Info,
               &eisInputInfo->expParams->data()->aecExpInfo,
               sizeof(eisInputInfo->expParams->data()->aecExpInfo));
        memcpy(&last_exp, &eisInputInfo->expParams->data()->aecExpInfo,
               sizeof(last_exp));
        LOGD_AEIS("eisAnalyze expo %f", last_exp.LinearExp.exp_real_params.integration_time);
    } else {
        memcpy(&eis_input_info->exp_Info, &last_exp, sizeof(last_exp));
        LOGD_ANALYZER("%s, id(%d), exposure is nullptr!",
                      __func__, nrStatsBufProxy->get_sequence ());
    }

    rk_aiq_nr_buf_info_t *nr_buf_info = (rk_aiq_nr_buf_info_t *)nrStatsBufProxy->get_private_info ();
    if (nr_buf_info) {
        memcpy(&eis_input_info->nr_buf_info, nr_buf_info, sizeof (*nr_buf_info));
        for (int32_t i = 0; i < nr_buf_info->buf_cnt; i++)
            LOGD_ANALYZER("nr buf cnt: %d, nrbuf_fd[%u]:%d, wxh: %dx%d",
                    eis_input_info->nr_buf_info.buf_cnt,
                    eis_input_info->nr_buf_info.fd_array[i],
                    eis_input_info->nr_buf_info.idx_array[i],
                    eis_input_info->nr_buf_info.width,
                    eis_input_info->nr_buf_info.height);
    }

    struct rkispp_stats_nrbuf *stats = nullptr;
    stats = (struct rkispp_stats_nrbuf *)(nrStatsBufProxy->get_v4l2_userptr());
    if (stats) {
        eis_input_info->nr_buf_info.nr_buf_index = stats->image.index;
        eis_input_info->nr_buf_info.nr_buf_size = stats->image.size;
        LOGD_ANALYZER("%s, id(%d), nr buf index: %d, size: %d", __func__,
                      eis_input_info->id,
                      eis_input_info->nr_buf_info.nr_buf_index,
                      eis_input_info->nr_buf_info.nr_buf_size);
    }

    fullParam = analyzeInternal(RK_AIQ_CORE_ANALYZE_AEIS);
    if (fullParam.ptr() && mCb) {
        //fullParam->data()->mFecParams->data()->frame_id = eis_input_info->id;
        //mCb->rkAiqCalcDone(fullParam);
    }
...
RkAiqCore::preProcess(enum rk_aiq_core_analyze_type_e type)
{
    ...
    if (type == RK_AIQ_CORE_ANALYZE_ALL || type == RK_AIQ_CORE_ANALYZE_AEIS) {
        PREPROCESS_ALGO(Aeis);
    }
#endif
...
int32_t
RkAiqCore::OnAlgosResCallback(RkAiqAlgoType_t type, RkAiqAlgoResCom* outparams)
{
    LOGD_ANALYZER("%s, type: 0x%x", __FUNCTION__, type);

    SmartPtr<RkAiqFullParamsProxy> aiqParamProxy = mAiqParamsPool->get_item();
    if (!aiqParamProxy.ptr()) {
        LOGE_ANALYZER("no free fec params buffer!");
    }
    RkAiqFullParams* aiqParams = aiqParamProxy->data().ptr();
    aiqParams->reset();

    if (type == RK_AIQ_ALGO_TYPE_AFEC) {
        if (mAiqFecParamsPool->has_free_items()) {
            aiqParams->mFecParams = mAiqFecParamsPool->get_item();
            aiqParams->mFecParams->data()->update_mask = 0;
        } else {
            LOGE_ANALYZER("no free fec params buffer!");
            return XCAM_RETURN_ERROR_MEM;
        }

        if (genIspAfecResult(aiqParams, outparams) < 0)
            LOGE_ANALYZER("Failed to get afec result!");
        else
            mAiqCurParams->data()->mFecParams = aiqParams->mFecParams;
    } else if (type == RK_AIQ_ALGO_TYPE_AEIS) {
        if (mAiqFecParamsPool->has_free_items()) {
            aiqParams->mFecParams = mAiqFecParamsPool->get_item();
            aiqParams->mFecParams->data()->update_mask = 0;
        } else {
        ...
        if (genIspAeisResult(aiqParams, outparams) < 0)
            LOGE_ANALYZER("Failed to get aeis result!");
        else
            mAiqCurParams->data()->mFecParams = aiqParams->mFecParams;
    } else {
        LOGE_ANALYZER("Don't match algo type: 0x%x", type);
    }

    if (aiqParamProxy.ptr() && mCb) {
        mCb->rkAiqCalcDone(aiqParamProxy);
    }
