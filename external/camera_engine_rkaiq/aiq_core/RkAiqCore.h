...
typedef struct RkAiqEisInputInfo_s {
    SmartPtr<RkAiqExpParamsProxy> expParams;
    SmartPtr<VideoBuffer> nrStatsBuf;
    uint64_t timestamp;
} RkAiqEisInputInfo;

class RkAiqCoreEisAnalyzerThread
    : public Thread {
public:
     RkAiqCoreEisAnalyzerThread(RkAiqCore* rkAiqCore)
        : Thread("RkAiqCoreEisAnalyzerThread")
        , mRkAiqCore(rkAiqCore) {};
    ~RkAiqCoreEisAnalyzerThread() {
        mBufQueue.clear ();
    };
    ...
    void triger_start() {
        mBufQueue.clear ();
        mBufQueue.resume_pop ();
    };

    bool push_buf (const SmartPtr<RkAiqEisInputInfo_s> &buffer) {
...
private:
    RkAiqCore* mRkAiqCore;
    SafeList<RkAiqEisInputInfo> mBufQueue;
};
...
class RkAiqCore {
    ...
    friend class RkAiqCoreEisAnalyzerThread;
public:
    RkAiqCore();
    virtual ~RkAiqCore();
    ...
    XCamReturn pushEisInputInfo (SmartPtr<VideoBuffer> &buffer,
                                 SmartPtr<RkAiqExpParamsProxy> &expParams,
                                 uint64_t timestamp);
...
public:
    ...
    typedef struct RkAiqAlgosShared_s {
        ...
        rk_aiq_eis_input_info eis_input_info;
        void reset() {
            ...
            eis_input_info.id = -1;
            eis_input_info.nr_buf_info.nr_buf_index = -1;
        }
    } RkAiqAlgosShared_t;
...
private:
    ...
    XCamReturn eisAnalyze(const SmartPtr<RkAiqEisInputInfo_s> &eisInputInfo);
    ...
    XCamReturn genIspAfecResult(RkAiqFullParams* params, RkAiqAlgoResCom* outparams);
    ...
    XCamReturn genIspAeisResult(RkAiqFullParams* params);
    XCamReturn genIspAeisResult(RkAiqFullParams* params, RkAiqAlgoResCom* outparams);
...
private:
    ...
    SmartPtr<RkAiqCoreEisAnalyzerThread> mRkAiqCoreEisAnalyzerTh;
    ...
    std::map<int, SmartPtr<RkAiqHandle>> mAeisAlgoHandleMap;
    ...
    SmartPtr<RkAiqHandle> mCurAeisAlgoHdl;
    ...
    SmartPtr<RkAiqTnrParamsPool> mAiqTnrParamsPool;
    SmartPtr<RkAiqFecParamsPool> mAiqFecParamsPool;
...
public:
    //archer.wu: dangerous modify here, for yushi's convenience
    ...
    int32_t OnAlgosResCallback(RkAiqAlgoType_t type, RkAiqAlgoResCom* outparams);
};
