...
RkAiqAeHandle::prepare()
{
    ...
    ae_config->dcg_ratio = shared->snsDes.dcg_ratio;
    ae_config->sony_brl = shared->snsDes.sony_brl;
...
RkAiqAeHandle::preProcess()
{
    ...
    // id != 0 means the thirdparty's algo
    if (mDes->id != 0 && shared->working_mode == RK_AIQ_WORKING_MODE_NORMAL) {
