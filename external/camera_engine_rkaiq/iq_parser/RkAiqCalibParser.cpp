...
(
    ...
    while (psubchild) {
        ...
        if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_AEC_DYDAMPEN_TAG_ID)) {
            uint8_t temp = mCalibDb->aec.CommCtrl.stAuto.stAeSpeed.DyDampEn;
            int no = ParseUcharArray(psubchild, &temp, subTag.Size());
            mCalibDb->aec.CommCtrl.stAuto.stAeSpeed.DyDampEn = (temp == 0) ? false : true;
            newparams = true;
            DCT_ASSERT((no == subTag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_AEC_SMOOTHEN_TAG_ID)) {
            uint8_t temp = mCalibDb->aec.CommCtrl.stAuto.stAeSpeed.SmoothEn;
            int no = ParseUcharArray(psubchild, &temp, subTag.Size());
            mCalibDb->aec.CommCtrl.stAuto.stAeSpeed.SmoothEn = (temp == 0) ? false : true;
            newparams = true;
            DCT_ASSERT((no == subTag.Size()));
        }
    ...
    if(newparams == false) {
        mCalibDb->aec.CommCtrl.stAuto.stAeSpeed.DyDampEn = true;
        mCalibDb->aec.CommCtrl.stAuto.stAeSpeed.SmoothEn = true;
    }
    XML_CHECK_END();
    autoTabBackward();
    LOGD("%s(%d): (exit)\n", __FUNCTION__, __LINE__);
...
(
    ...
    while (pchild) {
        ...
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_SENSORINFO_CISHDRTIMEREG_MAXSTEP_TAG_ID)) {
            int i = (sizeof(mCalibDb->sensor.CISHdrTimeRegMaxStep) / sizeof(mCalibDb->sensor.CISHdrTimeRegMaxStep.uCoeff[0]));
            int no = ParseUshortArray(pchild, mCalibDb->sensor.CISHdrTimeRegMaxStep.uCoeff, i);
            DCT_ASSERT((no == tag.Size()));
        }
...
(
    ...
    while (pchild) {
        ...
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_AHDR_TMO_AE_CONVERGED_THR_TAG_ID)) {
            int no = ParseFloatArray(pchild, &mCalibDb->ahdr.tmo.AeConvergedPdf, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        }
...
bool RkAiqCalibParser::parseEntrySensorLut3dModeCell
(
    const XMLElement*   pelement,
    void*               param
) {
    ...
    int index = *((int*)param);
    XML_CHECK_START(CALIB_SENSOR_LUT3D_MODE_CELL_TAG_ID, CALIB_SENSOR_LUT3D_TAG_ID);
    ...
    while (pchild) {
        ...
        if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_LUT3D_MODE_NAME_TAG_ID)) {
            ParseString(pchild,  mCalibDb->lut3d.mode_cell[index].name, sizeof(mCalibDb->lut3d.mode_cell[index].name));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_LUT3D_R_LUT_TAG_ID)) {
            int no = ParseUshortArray(pchild, mCalibDb->lut3d.mode_cell[index].look_up_table_r, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_LUT3D_G_LUT_TAG_ID)) {
            int no = ParseUshortArray(pchild, mCalibDb->lut3d.mode_cell[index].look_up_table_g, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_LUT3D_B_LUT_TAG_ID)) {
            int no = ParseUshortArray(pchild, mCalibDb->lut3d.mode_cell[index].look_up_table_b, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        }
        pchild = pchild->NextSibling();
...
(
    ...
    while (pchild) {
        ...
        else if(XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_LUT3D_MODE_CELL_TAG_ID)) {
            if (!parseEntryCell(pchild->ToElement(), tag.Size(),
                                &RkAiqCalibParser::parseEntrySensorLut3dModeCell,
                                param,
                                (uint32_t)CALIB_SENSOR_LUT3D_MODE_CELL_TAG_ID,
                                (uint32_t)CALIB_SENSOR_LUT3D_TAG_ID)) {
                LOGE("parse error in lut3d mode cell (%s)", tagname.c_str());
                return (false);
            }
        }else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_LUT3D_R_LUT_TAG_ID)) {
            int no = ParseUshortArray(pchild, mCalibDb->lut3d.mode_cell[0].look_up_table_r, tag.Size());
            strcpy(mCalibDb->lut3d.mode_cell[0].name,"normal");
            DCT_ASSERT((no == tag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_LUT3D_G_LUT_TAG_ID)) {
            int no = ParseUshortArray(pchild, mCalibDb->lut3d.mode_cell[0].look_up_table_g, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_LUT3D_B_LUT_TAG_ID)) {
            int no = ParseUshortArray(pchild, mCalibDb->lut3d.mode_cell[0].look_up_table_b, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        }
        pchild = pchild->NextSibling();
    }
...
(
    ...
    while (pchild) {
        ...
        if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_ENABLE_TAG_ID)) {
        ...
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECT_DS_SIZE_TAG_ID)) {
            int no = ParseIntArray(pchild, mCalibDb->mfnr.motion_detect_ds_size, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        }
            ...
            if (xmlParseReadWrite == XML_PARSER_READ) {
                ...
                mCalibDb->mfnr.mode_cell = (CalibDb_MFNR_ModeCell_V2_t *)malloc(cell_size * sizeof(CalibDb_MFNR_ModeCell_V2_t));
                memset(mCalibDb->mfnr.mode_cell, 0x00, cell_size * sizeof(CalibDb_MFNR_ModeCell_V2_t));
                if (!parseEntryCell(pchild->ToElement(), tag.Size(),
...
(
    ...
    while (psubchild) {
        ...
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_DYNAMIC_EN_TAG_TD)) {
            int no = ParseIntArray(psubchild, &mCalibDb->mfnr.mode_cell[index].motion.dynamicEnable, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_DYNAMICSW_TAG_ID)) {
            int no = ParseFloatArray(psubchild, mCalibDb->mfnr.mode_cell[index].motion.dynamicSw, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        ...
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_GAIN_UPD_FLG_TAG_ID)) {
            int no = ParseIntArray(psubchild, &mCalibDb->mfnr.mode_cell[index].motion.gainUpdFlag, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_MD_SIGMA_SCALE_TAG_ID)) {
            int no = ParseFloatArray(psubchild, mCalibDb->mfnr.mode_cell[index].motion.mdSigmaScale, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_MED_EN_TAG_ID)) {
            int no = ParseIntArray(psubchild, &mCalibDb->mfnr.mode_cell[index].motion.medEn, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_YUVNR_GAIN_SCALE3_TAG_ID)) {
            int no = ParseFloatArray(psubchild, mCalibDb->mfnr.mode_cell[index].motion.yuvnrGainScale3, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_YUVNR_GAIN_GLB_TAG_ID)) {
            int no = ParseFloatArray(psubchild, mCalibDb->mfnr.mode_cell[index].motion.yuvnrGainGlb, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_YUVNR_GAIN_TH_TAG_ID)) {
            int no = ParseFloatArray(psubchild, mCalibDb->mfnr.mode_cell[index].motion.yuvnrGainTH, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_MFNR_GAIN_SCALE_TAG_ID)) {
            int no = ParseFloatArray(psubchild, mCalibDb->mfnr.mode_cell[index].motion.mfnrGainScale, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_MOTION_DN_STR_TAG_ID)) {
            int no = ParseFloatArray(psubchild, mCalibDb->mfnr.mode_cell[index].motion.mfnrDnStr, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_THRETH01_TAG_ID)) {
            int no = ParseFloatArray(psubchild, mCalibDb->mfnr.mode_cell[index].motion.thresh01, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_MFNR_MOTION_DETECTION_THRETH23_TAG_ID)) {
            int no = ParseFloatArray(psubchild, mCalibDb->mfnr.mode_cell[index].motion.thresh23, subTag.Size());
            DCT_ASSERT((no == subTag.Size()));
        }
        else {
...
(
    ...
    CalibDb_MFNR_ModeCell_V2_t *pModeCell = (CalibDb_MFNR_ModeCell_V2_t *)param;
...
(
    ...
    while (pchild) {
        ...
        if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_ENABLE_TAG_ID)) {
            int no = ParseUcharArray(pchild, &mCalibDb->eis_calib.enable, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_MODE_TAG_ID)) {
            int no = ParseIntArray(pchild, static_cast<int*>(&mCalibDb->eis_calib.mode), tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_XML_PATH_TAG_ID)) {
            int no = ParseString(pchild, mCalibDb->eis_calib.debug_xml_path,
                        sizeof(mCalibDb->eis_calib.debug_xml_path));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_CAMERA_MODEL_TAG_ID)) {
            int no = ParseIntArray(pchild, &mCalibDb->eis_calib.camera_model_type, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_CAMERA_RATE_TAG_ID)) {
            int no = ParseDoubleArray(pchild, &mCalibDb->eis_calib.camera_rate, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_IMU_RATE_TAG_ID)) {
            int no = ParseDoubleArray(pchild, &mCalibDb->eis_calib.imu_rate, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_IMAGE_BUFFER_NUM_TAG_ID)) {
            int no = ParseUintArray(pchild, &mCalibDb->eis_calib.image_buffer_num, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_SRC_WIDTH_TAG_ID)) {
            int no = ParseUintArray(pchild, &mCalibDb->eis_calib.src_image_width, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_SRC_HEIGHT_TAG_ID)) {
            int no = ParseUintArray(pchild, &mCalibDb->eis_calib.src_image_height, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_DST_WIDTH_TAG_ID)) {
            int no = ParseUintArray(pchild, &mCalibDb->eis_calib.dst_image_width, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_DST_HEIGHT_TAG_ID)) {
            int no = ParseUintArray(pchild, &mCalibDb->eis_calib.dst_image_height, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_CLIP_RATIO_X_TAG_ID)) {
            int no = ParseFloatArray(pchild, &mCalibDb->eis_calib.clip_ratio_x, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_CLIP_RATIO_Y_TAG_ID)) {
            int no = ParseFloatArray(pchild, &mCalibDb->eis_calib.clip_ratio_y, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_FOCAL_LENGTH_TAG_ID)) {
            int no = ParseDoubleArray(pchild, mCalibDb->eis_calib.focal_length, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_LIGHT_CENTER_TAG_ID)) {
            int no = ParseDoubleArray(pchild, mCalibDb->eis_calib.light_center, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_DISTORTION_COEFF_TAG_ID)) {
            int no = ParseDoubleArray(pchild, mCalibDb->eis_calib.distortion_coeff, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_XI_TAG_ID)) {
            int no = ParseDoubleArray(pchild, &mCalibDb->eis_calib.xi, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_GBIAS_X_TAG_ID)) {
            int no = ParseDoubleArray(pchild, &mCalibDb->eis_calib.gbias_x, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_GBIAS_Y_TAG_ID)) {
            int no = ParseDoubleArray(pchild, &mCalibDb->eis_calib.gbias_y, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_GBIAS_Z_TAG_ID)) {
            int no = ParseDoubleArray(pchild, &mCalibDb->eis_calib.gbias_z, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_TIME_OFFSET_TAG_ID)) {
            int no = ParseDoubleArray(pchild, &mCalibDb->eis_calib.time_offset, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_SENSOR_AXIS_TYPE_TAG_ID)) {
            int no = ParseIntArray(pchild, &mCalibDb->eis_calib.sensor_axes_type, tag.Size());
            DCT_ASSERT((no == tag.Size()));
        } else if (XML_CHECK_TAGID_COMPARE(CALIB_SENSOR_EIS_SAVE_OBJPTS_PATH_TAG_ID)) {
            int no = ParseString(pchild, mCalibDb->eis_calib.save_objpts_path,
                                 sizeof(mCalibDb->eis_calib.save_objpts_path));
        }
        pchild = pchild->NextSibling();
    }
    XML_CHECK_END();
