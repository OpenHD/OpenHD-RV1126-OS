...
class nrStatsListener {
public:
    nrStatsListener () {};
    virtual ~nrStatsListener () {};
    virtual XCamReturn nrStatsCb(SmartPtr<VideoBuffer>& nrStats) = 0;
private:
    XCAM_DEAD_COPY (nrStatsListener);
};
...
public:
    ICamHw() {};
    virtual ~ICamHw() {};
    virtual XCamReturn set_read_back_mode(bool on) = 0;
    virtual XCamReturn init(const char* sns_ent_name) = 0;
    virtual XCamReturn deInit() = 0;
    virtual XCamReturn set_tx_buf_num(uint16_t buf_num) = 0;
    ...
    virtual void getIspWorkMode(int& mode) = 0;
    ...
    virtual XCamReturn setFecParams(SmartPtr<RkAiqFecParamsProxy>& fecParams) = 0;
    virtual XCamReturn setTnrParams(SmartPtr<RkAiqTnrParamsProxy>& tnrParams) = 0;
    ...
    virtual XCamReturn setNrStatsListener(nrStatsListener* nrStatsListener) = 0;
    ...
    virtual XCamReturn get_rkraw_frame_info(rkraw_frame_info_t *frame_info) = 0;
    ...
    virtual XCamReturn getEffectiveExpParams(uint32_t id, SmartPtr<RkAiqExpParamsProxy>& expParams, uint64_t &timestamp) = 0;
private:
