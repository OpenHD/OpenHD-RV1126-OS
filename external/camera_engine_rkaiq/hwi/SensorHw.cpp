...
XCamReturn
SensorHw::get_readout_linecnt_perline (rk_aiq_exposure_sensor_descriptor* sns_des)
{
    if (io_control(RKMODULE_GET_READOUT_LINE_CNT_PER_LINE, &sns_des->sensor_readout_linecnt_perline) < 0) {
       LOGV_CAMHW_SUBM(SENSOR_SUBM, "get_readout_linecnt_perline returned: (%s)", strerror(errno));
       return  XCAM_RETURN_ERROR_IOCTL;
    }

    LOGV_CAMHW_SUBM(SENSOR_SUBM, "readout_linecnt_perline: %d", sns_des->sensor_readout_linecnt_perline);
...
int
SensorHw::get_sony_brl(rk_aiq_exposure_sensor_descriptor* sns_des)
{
    uint32_t sony_brl = 0;

    if (io_control(RKMODULE_GET_SONY_BRL, &sony_brl) < 0) {
        //LOGD_CAMHW_SUBM(SENSOR_SUBM,"failed to get sensor sony brl");
        sns_des->sony_brl = 0;
        return XCAM_RETURN_ERROR_IOCTL;
    }

    sns_des->sony_brl = sony_brl;
...
SensorHw::get_sensor_descriptor(rk_aiq_exposure_sensor_descriptor *sns_des)
{
    ...
    if (get_readout_linecnt_perline (sns_des))
        return XCAM_RETURN_ERROR_IOCTL;
                            ...
                            rk_aiq_exposure_sensor_descriptor& sns_des)
{
    rk_aiq_exposure_sensor_descriptor sensor_desc;
    get_sensor_descriptor (&sensor_desc);
    get_sony_brl(&sensor_desc);
    get_dcg_ratio(&sensor_desc.dcg_ratio);
    ...
    sns_des.sony_brl = sensor_desc.sony_brl;
    ...
    sns_des.sensor_readout_linecnt_perline = sensor_desc.sensor_readout_linecnt_perline;
