...
void
Isp20Params::convertAiqTnrResultsToTnrParams(struct rkispp_params_tnrcfg &tnr_cfg,
                                             SmartPtr<RkAiqTnrParamsProxy>& tnr_params)
{
    LOGD_CAMHW_SUBM(ISP20PARAM_SUBM, "%s:(%d) enter \n", __FUNCTION__, __LINE__);
    int i = 0;
    rk_aiq_isp_tnr_t *tnr = &tnr_params->data()->tnr;

    LOGD_CAMHW_SUBM(ISP20PARAM_SUBM, "tnr_en %d", tnr->tnr_en);

    if(tnr->tnr_en) {
        tnr_cfg.head.module_ens |= ISPP_MODULE_TNR;
    } else {
        //tnr_cfg.module_init_ens &= ~ISPP_MODULE_TNR_3TO1;
        tnr_cfg.head.module_ens &= ~ISPP_MODULE_TNR;
    }

    tnr_cfg.head.module_en_update |= ISPP_MODULE_TNR;
    tnr_cfg.head.module_cfg_update |= ISPP_MODULE_TNR;

    struct rkispp_tnr_config  * pTnrCfg = &tnr_cfg.tnr_cfg;

    //0x0080
    if (tnr->mode > 0) {
        tnr_cfg.head.module_ens |= ISPP_MODULE_TNR_3TO1;
    } else {
        tnr_cfg.head.module_ens |= ISPP_MODULE_TNR;
    }

    LOGD_CAMHW_SUBM(ISP20PARAM_SUBM, "mode:%d  tnr_cfg:0x%x\n", tnr->mode, tnr_cfg.head.module_ens);
...
void
Isp20Params::convertAiqUvnrToIsp20Params(struct rkispp_params_nrcfg &nr_cfg,
                                         rk_aiq_isp_uvnr_t& uvnr)
{
    LOGD_CAMHW_SUBM(ISP20PARAM_SUBM, "%s:(%d) enter \n", __FUNCTION__, __LINE__);

    int i = 0;
    struct rkispp_nr_config  * pNrCfg = &nr_cfg.nr_cfg;

    LOGD_CAMHW_SUBM(ISP20PARAM_SUBM, "uvnr_en %d", uvnr.uvnr_en);
    if(uvnr.uvnr_en) {
        nr_cfg.head.module_ens |= ISPP_MODULE_NR;
        // pp_cfg.module_init_ens |= ISPP_MODULE_NR;
    } else {
        nr_cfg.head.module_ens &=  ~ISPP_MODULE_NR;
    }

    nr_cfg.head.module_en_update |= ISPP_MODULE_NR;
    nr_cfg.head.module_cfg_update |= ISPP_MODULE_NR;
...
void
Isp20Params::convertAiqYnrToIsp20Params(struct rkispp_params_nrcfg &nr_cfg,
                                        rk_aiq_isp_ynr_t& ynr)
{
    ...
    if(ynr.ynr_en) {
        nr_cfg.head.module_ens |= ISPP_MODULE_NR;
        // nr_cfg.head.module_init_ens |= ISPP_MODULE_NR;
    } else {
        nr_cfg.head.module_ens &= ~ISPP_MODULE_NR;
    }
...
void
Isp20Params::convertAiqSharpenToIsp20Params(struct rkispp_params_nrcfg &nr_cfg,
        rk_aiq_isp_sharpen_t& sharp, rk_aiq_isp_edgeflt_t& edgeflt)
{
    int i = 0;
    struct rkispp_sharp_config  * pSharpCfg = &nr_cfg.shp_cfg;
    ...
    if(pSharpV1->sharp_en && edgeflt.edgeflt_en) {
        nr_cfg.head.module_ens |= ISPP_MODULE_SHP;
        // pp_cfg.module_init_ens |= ISPP_MODULE_SHP;
    } else {
        nr_cfg.head.module_ens &=  ~ISPP_MODULE_SHP;
    }

    nr_cfg.head.module_en_update |= ISPP_MODULE_SHP;
    nr_cfg.head.module_cfg_update |= ISPP_MODULE_SHP;
#if 1
    //0x0080
...
void
Isp20Params::convertAiqFecToIsp20Params(struct rkispp_params_feccfg& fec_cfg,
                                        SmartPtr<RkAiqFecParamsProxy>& fecParams)
{
    ...
    rk_aiq_isp_fec_t *fec = &fecParams->data()->fec;
    LOGD_CAMHW_SUBM(ISP20PARAM_SUBM, "fec update params, enable %d ", fec->fec_en);
    if(fec->fec_en) {
        if (fec->usage == ISPP_MODULE_FEC_ST) {
            fec_cfg.head.module_ens |= ISPP_MODULE_FEC_ST;
            fec_cfg.head.module_en_update |= ISPP_MODULE_FEC_ST;
        } else if (fec->usage == ISPP_MODULE_FEC) {
            fec_cfg.head.module_ens |= ISPP_MODULE_FEC;
            fec_cfg.head.module_en_update |= ISPP_MODULE_FEC;
        }

        if (!fec->config) {
            if (fec->usage == ISPP_MODULE_FEC_ST)
                fec_cfg.head.module_cfg_update &= ~ISPP_MODULE_FEC_ST;
            else if (fec->usage == ISPP_MODULE_FEC)
                fec_cfg.head.module_cfg_update &= ~ISPP_MODULE_FEC;
        } else {
            if (fec->usage == ISPP_MODULE_FEC_ST)
                fec_cfg.head.module_cfg_update |= ISPP_MODULE_FEC_ST;
            else if (fec->usage == ISPP_MODULE_FEC)
            ...
            struct rkispp_fec_config *pFecCfg   = &fec_cfg.fec_cfg;
            ...
            fec_cfg.image.index                 = fec->img_buf_index;
            fec_cfg.image.size                  = fec->img_buf_size;
        }
    } else {
        fec_cfg.head.module_ens &= ~(ISPP_MODULE_FEC_ST | ISPP_MODULE_FEC);
        fec_cfg.head.module_en_update &= ~(ISPP_MODULE_FEC_ST | ISPP_MODULE_FEC);
    }
}
...
XCamReturn
Isp20Params::convertAiqNrResultsToNrParams(struct rkispp_params_nrcfg &nr_cfg,
        SmartPtr<RkAiqNrParamsProxy>& nr_params)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    if(nr_params->data()->update_mask & ISPP_MODULE_NR) {
        convertAiqYnrToIsp20Params(nr_cfg, nr_params->data()->ynr);
        convertAiqUvnrToIsp20Params(nr_cfg, nr_params->data()->uvnr);
    }

    if(nr_params->data()->update_mask & ISPP_MODULE_SHP)
        convertAiqSharpenToIsp20Params(nr_cfg, nr_params->data()->sharpen,
                                       nr_params->data()->edgeflt);
...
{
    LOG1_AHDR("%s: PixelNumBlock:%d blc:%f\n", __FUNCTION__, PixelNumBlock, blc);
...
Isp20Params::hdrtmoGetAeInfo(RKAiqAecExpInfo_t* Next, RKAiqAecExpInfo_t* Cur, s32 frameNum, float* expo)
{
    ...
    LOG1_AHDR("%s: Cur Expo: S:%f M:%f L:%f\n", __FUNCTION__, curSExpo, curMExpo, curLExpo);
    LOG1_AHDR("%s: Next Expo: S:%f M:%f L:%f\n", __FUNCTION__, nextSExpo, nextMExpo, nextLExpo);

}

bool
Isp20Params::hdrtmoSceneStable(sint32_t frameId, TmoFlickerPara_t* pTmoFlicker, s32 frameNum)
{
    ...
    if(frameNum == 1) {
        LumaDeviationLinear = pTmoFlicker->LumaDeviation[0];
        LumaDeviationFinnal = LumaDeviationLinear;
    }
    else if(frameNum == 2) {
        LumaDeviationS = pTmoFlicker->LumaDeviation[0];
        LumaDeviationL = pTmoFlicker->LumaDeviation[1];
        ...
        LumaDeviationS = pTmoFlicker->LumaDeviation[0];
        LumaDeviationM = pTmoFlicker->LumaDeviation[1];
        LumaDeviationL = pTmoFlicker->LumaDeviation[2];
    ...
    LOG1_AHDR("%s: frameId:%ld LumaDeviationLinear:%f LumaDeviationS:%f LumaDeviationM:%f LumaDeviationL:%f\n", __FUNCTION__,
    ...
    if(AntiTmoFlicker.FirstChangeDoneNum && AntiTmoFlicker.FirstChangeNum) {
        if(pTmoFlicker->AeConvergedPdf >= 0.1) {
            if(LumaDeviationFinnal <= pTmoFlicker->AeConvergedPdf)
                SceneStable = true;
            else
                SceneStable = false;
        }
        else
            SceneStable = pTmoFlicker->IsAeConverged;
    }
    else
        SceneStable = true;

    LOG1_AHDR("%s: preFrameNum:%d frameNum:%d FirstChange:%d FirstChangeNum:%d FirstChangeDone:%d FirstChangeDoneNum:%d\n", __FUNCTION__,
    ...
    LOG1_AHDR("%s: LumaDeviationFinnal:%f AeConvergedPdf:%f IsAeConverged:%d SceneStable:%d \n", __FUNCTION__, LumaDeviationFinnal, pTmoFlicker->AeConvergedPdf,
              pTmoFlicker->IsAeConverged, SceneStable);
...
Isp20Params::hdrtmoPredictK(float* luma, float* expo, s32 frameNum, PredictKPara_t *TmoPara)
{
    ...
    LOG1_AHDR("%s: nextLLuma:%f curLLuma:%f\n", __FUNCTION__, nextLMeanLuma, curLMeanLuma);
    LOG1_AHDR("%s: nextSLuma:%f curSLuma:%f\n", __FUNCTION__, nextSMeanLuma, curSMeanLuma);
    ...
    if (frameNum == 1)
    {
        LOG1_AHDR("%s: nextLuma:%f curLuma:%f LumaChange:%f\n", __FUNCTION__, nextSMeanLuma, curSMeanLuma, ShortLumaChange);
        ...
        LOG1_AHDR("%s: nextEnvLv:%f curEnvLv:%f EnvLvChange:%f\n", __FUNCTION__, nextSMeanLuma / nextSExpo,
                  curSMeanLuma / curSExpo, EnvLvChange);
    }
    else if (frameNum == 2)
    {
        LOG1_AHDR("%s: nextLLuma:%f curLLuma:%f LongLumaChange:%f\n", __FUNCTION__, nextLMeanLuma, curLMeanLuma, LongLumaChange);
        LOG1_AHDR("%s: nextSLuma:%f curSLuma:%f ShortLumaChange:%f\n", __FUNCTION__, nextSMeanLuma, curSMeanLuma, ShortLumaChange);
        LOG1_AHDR("%s: LongPercent:%f UseLongLowTh:%f UseLongUpTh:%f\n", __FUNCTION__, 1, TmoPara->UseLongLowTh, TmoPara->UseLongUpTh);
        ...
        LOG1_AHDR("%s: nextEnvLv:%f curEnvLv:%f EnvLvChange:%f\n", __FUNCTION__, nextLMeanLuma / nextLExpo,
    ...
    {
        LOG1_AHDR("%s: nextLLuma:%f curLLuma:%f LongLumaChange:%f\n", __FUNCTION__, nextLMeanLuma, curLMeanLuma, LongLumaChange);
        LOG1_AHDR("%s: nextMLuma:%f curMLuma:%f MiddleLumaChange:%f\n", __FUNCTION__, nextMMeanLuma, curMMeanLuma, MiddleLumaChange);
        LOG1_AHDR("%s: nextSLuma:%f curSLuma:%f ShortLumaChange:%f\n", __FUNCTION__, nextSMeanLuma, curSMeanLuma, ShortLumaChange);
        LOG1_AHDR("%s: LongPercent:%f UseLongLowTh:%f UseLongUpTh:%f\n", __FUNCTION__, TmoPara->Hdr3xLongPercent,
        ...
        LOG1_AHDR("%s: nextEnvLv:%f curEnvLv:%f EnvLvChange:%f\n", __FUNCTION__, nextMMeanLuma / nextMExpo,
    ...
    LOG1_AHDR("%s: ratio:%f EnvLvChange:%f PredictKfloat:%f PredictK:%d\n", __FUNCTION__,
              ratio, EnvLvChange, PredictKfloat, PredictK);
    return PredictK;
}

s16
Isp20Params::CalcSetLgmean(sint32_t frameId, PredictPara_t* pPredict, TmoFlickerPara_t* pTmoFlicker)
{
    unsigned short default_lgmean = 40000;
    unsigned short lgmean = default_lgmean;

    if (pPredict->iir < pPredict->iir_max) {
        unsigned short ro_lgmean = pTmoFlicker->ro_hdrtmo_lgmean;
        unsigned short iir = 0;
        unsigned short global_tmo_strength = pPredict->global_tmo_strength;

        iir = MIN(frameId + 1, pPredict->iir);
        default_lgmean += global_tmo_strength;
        ro_lgmean +=  global_tmo_strength;
        if (pPredict->Scenestable) {
            if (frameId == 0)
                lgmean = default_lgmean;
            else
                lgmean = ((iir - 1) * AntiTmoFlicker.prev_lgmean + ro_lgmean) / iir;
        } else {
            if (frameId == 0)
                lgmean = default_lgmean;
            else
                lgmean = AntiTmoFlicker.prev_lgmean;
        }
    }

    AntiTmoFlicker.prev_lgmean = lgmean;

    LOG1_AHDR("%s: frameId:%d iir:%d iir_max:%d scene_stable:%d lgmean:%d\n",  __FUNCTION__, frameId,
              pPredict->iir, pPredict->iir_max, pPredict->Scenestable, lgmean);
    return lgmean;
}

s16
Isp20Params::CalcSetPalhpa(TmoProcRes_t* pTmoProc, PredictPara_t* pPredict, TmoFlickerPara_t* pTmoFlicker, unsigned short lgmean)
{
    unsigned short palpha = 0;
    if(pPredict->Scenestable) {
        unsigned short set_lgmin = 0, set_lgmax = 0, palpha_0p18 = 0, max_palpha = 0, damp = 0;

        set_lgmin = pTmoProc->sw_hdrtmo_set_lgmin;
        set_lgmax = pTmoProc->sw_hdrtmo_set_lgmax;
        palpha_0p18 = pTmoProc->sw_hdrtmo_palpha_0p18;
        max_palpha = pTmoProc->sw_hdrtmo_maxpalpha;
        damp = (int)(1000 * pTmoFlicker->damp);

        palpha = palpha_0p18 * (4 * lgmean - 3 * set_lgmin - set_lgmax) / (set_lgmax - set_lgmin);
        palpha = MIN(palpha, max_palpha);
        palpha = (damp * palpha + (1000 - damp) * AntiTmoFlicker.prev_palpha) / 1000;

    }
    else
        palpha = AntiTmoFlicker.prev_palpha;

    LOG1_AHDR("%s: Scenestable:%d prev_palpha:%d damp:%f palpha:%d\n",  __FUNCTION__, pPredict->Scenestable, AntiTmoFlicker.prev_palpha, pTmoFlicker->damp, palpha);

    AntiTmoFlicker.prev_palpha = palpha;

    return palpha;
}

s16
Isp20Params::CalcSetLgrange0(TmoProcRes_t* pTmoProc, float lgmin, float lgmax)
{
    unsigned short lgrange0 = 0;

    float value = 0.0;
    float clipratio0 = pTmoProc->sw_hdrtmo_clipratio0 / 256.0;
    float clipgap0 = pTmoProc->sw_hdrtmo_clipgap0 / 4.0;
    value = lgmin * (1 - clipratio0) + lgmax * clipratio0;
    value = MIN(value, (lgmin + clipgap0));
    lgrange0 = (int)(2048 * value);

    return lgrange0;
}

s16
Isp20Params::CalcSetLgrange1(TmoProcRes_t* pTmoProc, float lgmin, float lgmax, unsigned short lgmean)
{
    unsigned short lgrange1 = 0;

    float value = 0.0;
    float clipratio1 = pTmoProc->sw_hdrtmo_clipratio1 / 256.0;
    float clipgap1 = pTmoProc->sw_hdrtmo_clipgap1 / 4.0;
    value = lgmin * (1 - clipratio1) + lgmax * clipratio1;
    value = MAX(value, (lgmax - clipgap1));
    lgrange1 = (int)(2048 * value);

    if(pTmoProc->sw_hdrtmo_set_weightkey == 0)
        lgrange1 = lgmean;
...
s16
Isp20Params::CalcSetLgavgmax(TmoProcRes_t* pTmoProc, unsigned short lgmean)
{
    unsigned char weight_key = 0;
    unsigned short set_lgmax = 0, lgrange1 = 0;
    unsigned int lgavgmax = 0;

    set_lgmax = pTmoProc->sw_hdrtmo_set_lgmax;
    lgrange1 = pTmoProc->sw_hdrtmo_set_lgrange1;
    weight_key = pTmoProc->sw_hdrtmo_set_weightkey;
    ...
    LOG1_AHDR("%s: set_lgmax3:%d lgrange1:%d weight_key:%d lgmean:%d lgavgmax:%d\n",  __FUNCTION__, set_lgmax,
              lgrange1, weight_key, lgmean, lgavgmax);
