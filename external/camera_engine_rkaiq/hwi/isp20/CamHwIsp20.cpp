...
CamHwIsp20::CamHwIsp20()
    ...
    , _dvp_itf(false)
    , _isp_tx_buf_num(ISP_TX_BUF_NUM)
    , _vicap_tx_buf_num(VICAP_TX_BUF_NUM)
{
...
XCamReturn CamHwIsp20::set_read_back_mode(bool on)
{
    if (_linked_to_isp) {
        mNormalNoReadBack = !on;
...
rk_aiq_static_info_t* CamHwIsp20::getStaticCamHwInfoByPhyId(
    const char* sns_ent_name, uint16_t index) {
    if (sns_ent_name) {
        std::string str(sns_ent_name);

        auto it = mCamHwInfos.find(str);
        if (it != mCamHwInfos.end()) {
            LOGD_CAMHW_SUBM(ISP20HW_SUBM, "find camerainfo of %s!",
                            sns_ent_name);
            return it->second.ptr();
        } else {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "camerainfo of %s not fount!",
                            sns_ent_name);
        }
    } else {
        ...
        auto it = std::find_if(
            ...
            [&](const std::pair<std::string, SmartPtr<rk_aiq_static_info_t>>&
                    info) -> bool {
...
XCamReturn
CamHwIsp20::set_tx_buf_num(uint16_t buf_num) {
    _isp_tx_buf_num = buf_num;
    _vicap_tx_buf_num = buf_num;

    for (int i = 0; i < 3; i++) {
        if (_linked_to_isp) {
            if (_mipi_tx_devs[i].ptr())
                _mipi_tx_devs[i]->set_buffer_count(_isp_tx_buf_num);
            if (_mipi_rx_devs[i].ptr())
                _mipi_rx_devs[i]->set_buffer_count(_isp_tx_buf_num);
        } else {
            if (_mipi_tx_devs[i].ptr())
                _mipi_tx_devs[i]->set_buffer_count(_vicap_tx_buf_num);
            if (_mipi_rx_devs[i].ptr())
                _mipi_rx_devs[i]->set_buffer_count(_vicap_tx_buf_num);
        }
        if (_mipi_tx_devs[i].ptr())
...
CamHwIsp20::init(const char* sns_ent_name)
{
    ...
    mIsppStatsDev->open();
#endif
    if (strlen(s_info->ispp_info->pp_input_params_path)) {
        mIsppParamsDev = new V4l2Device (s_info->ispp_info->pp_input_params_path);
        mIsppParamsDev->open();
    }

#ifdef DISABLE_PP
    if (strlen(s_info->ispp_info->pp_tnr_params_path)) {
         mTnrProcUint = new TnrProcUint ();
         /* TNR stats need to be output only when the motion detection is opened*/
         if ((mCalibDb->mfnr.enable && mCalibDb->mfnr.motion_detect_en) || mCalibDb->af.ldg_param.enable) {
             mTnrProcUint->init (s_info, true);
             mTnrProcUint->set_ispp_subdevice (_ispp_sd);
         } else
             mTnrProcUint->init (s_info, false);
         mTnrProcUint->set_poll_callback (this);
         mTnrProcUint->set_params_translator (this);
    }

    if (strlen (s_info->ispp_info->pp_nr_stats_path) && \
        strlen(s_info->ispp_info->pp_nr_params_path)) {
         mNrProcUint = new NrProcUint ();
         mNrProcUint->set_ispp_subdevice (_ispp_sd);
         if (mCalibDb->eis_calib.enable) {
             mNrProcUint->set_eis_status (true);
         } else {
             mNrProcUint->set_eis_status (false);
         }

         if ((mCalibDb->mfnr.enable && mCalibDb->mfnr.motion_detect_en) || mCalibDb->af.ldg_param.enable) {
             mNrProcUint->init (s_info);
             mNrProcUint->set_amd_status (true);
         } else {
             mNrProcUint->init (s_info);
             mNrProcUint->set_amd_status (false);
         }
         mNrProcUint->set_poll_callback (this);
         mNrProcUint->set_params_translator (this);
	}

    if (strlen (s_info->ispp_info->pp_fec_params_path)) {
         mFecProcUint = new FecProcUint ();
         mFecProcUint->init (s_info);
	}
#endif
    ...
    /*
     * if (mNrProcUint.ptr ())
     *     mNrProcUint->set_poll_thread (mPollthread);
     */
                                ...
                                __u32 sns_v4l_pix_fmt)
{
    ...
    for (int i = 0; i < 3; i++) {
        if (_mipi_tx_devs[i].ptr()) {
            ...
            if (_dvp_itf && (sns_v4l_pix_fmt != V4L2_PIX_FMT_GREY) &&
                (sns_v4l_pix_fmt != V4L2_PIX_FMT_SBGGR8) &&
                (sns_v4l_pix_fmt != V4L2_PIX_FMT_SGBRG8) &&
                (sns_v4l_pix_fmt != V4L2_PIX_FMT_SGRBG8) &&
                (sns_v4l_pix_fmt != V4L2_PIX_FMT_SRGGB8)) {
                int mem_mode = CSI_LVDS_MEM_WORD_HIGH_ALIGN;
                int ret1 = _mipi_tx_devs[i]->io_control(
                    RKCIF_CMD_SET_CSI_MEMORY_MODE, &mem_mode);
                if (ret1)
                    LOGE_CAMHW_SUBM(
                        ISP20HW_SUBM,
                        "set CSI_LVDS_MEM_WORD_HIGH_ALIGN failed !\n");
            }
            ...
            if (format.fmt.pix.width != sns_sd_sel.r.width ||
                ...
                _mipi_tx_devs[i]->set_format(
                    sns_sd_sel.r.width, sns_sd_sel.r.height, sns_v4l_pix_fmt,
                    V4L2_FIELD_NONE, 0);
            }
        }
        ...
        if (_mipi_rx_devs[i].ptr()) {
            ...
            if (_dvp_itf && (sns_v4l_pix_fmt != V4L2_PIX_FMT_GREY) &&
                ...
                int ret1 = _mipi_rx_devs[i]->io_control(
                    RKISP_CMD_SET_CSI_MEMORY_MODE, &mem_mode);
                if (ret1)
                    LOGE_CAMHW_SUBM(ISP20HW_SUBM,
                                    "set CSI_MEM_WORD_BIG_ALIGN failed !\n");
            }
            if (format.fmt.pix.width != sns_sd_sel.r.width ||
                format.fmt.pix.height != sns_sd_sel.r.height ||
                format.fmt.pix.pixelformat != sns_v4l_pix_fmt) {
                if (_mipi_rx_devs[i].ptr())
                    _mipi_rx_devs[i]->set_format(
                        sns_sd_sel.r.width, sns_sd_sel.r.height,
                        sns_v4l_pix_fmt, V4L2_FIELD_NONE, 0);
            }
        }
    }
                                ...
                                __u32 sns_v4l_pix_fmt)
{
    ...
    if (_linked_to_isp && (_crop_rect.width || _crop_rect.height)) {
        aSelection.r.width = _crop_rect.width;
        aSelection.r.height = _crop_rect.height;
        aSelection.r.left = _crop_rect.left;
        aSelection.r.top = _crop_rect.top;
    } else {
        aSelection.r.width = sns_sd_sel.r.width;
        aSelection.r.height = sns_sd_sel.r.height;
...
CamHwIsp20::setupPipelineFmt()
{
    ...
    if (!_linked_to_isp && (_crop_rect.width || _crop_rect.height)) {
        ...
        sns_sd_sel.r.left = _crop_rect.left;
        sns_sd_sel.r.top = _crop_rect.top;
...
CamHwIsp20::start()
{
    LOGD_CAMHW_SUBM(ISP20HW_SUBM, ">>>>>>>>>>>>>>>>>>>>>> START <<<<<<<<<<<<<<<<<<<<<<<\n");
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    SmartPtr<BaseSensorHw> sensorHw;
    SmartPtr<LensHw> lensHw;
    SmartPtr<Isp20PollThread> isp20Pollthread;
    ...
    LOGD_CAMHW_SUBM(ISP20HW_SUBM, "mPpModuleInitEns: 0x%x\n", mPpModuleInitEns);
    ...
    if (mTnrProcUint.ptr ()) {
        ret = mTnrProcUint->start ();
        if (ret < 0) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "tnr start err: %d\n", ret);
            return ret;
        }
    }
    if (mNrProcUint.ptr ()) {
        ret = mNrProcUint->start ();
        if (ret < 0) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "nr start err: %d\n", ret);
            return ret;
        }
    }
    if (mFecProcUint.ptr ()) {
        ret = mFecProcUint->start ();
        if (ret < 0) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "fec start err: %d\n", ret);
            return ret;
        }
    }
    if (mFlashLight.ptr()) {
    ...
    EXIT_CAMHW_FUNCTION();
    LOGD_CAMHW_SUBM(ISP20HW_SUBM, ">>>>>>>>>>>>>>>>>>>>>> STARTED <<<<<<<<<<<<<<<<<<<<<<<\n");
    return ret;
}
...
{
    LOGD_CAMHW_SUBM(ISP20HW_SUBM, ">>>>>>>>>>>>>>>>>>>>>> STOP <<<<<<<<<<<<<<<<<<<<<<<\n");
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    SmartPtr<Isp20PollThread> isp20Pollthread;
#ifndef DISABLE_ISP20SPTHREAD
    SmartPtr<Isp20SpThread> isp20Spthread;
    ...
    if (_state != CAM_HW_STATE_STARTED) {
        LOGE_CAMHW_SUBM(ISP20HW_SUBM, "camhw state err: %d\n", ret);
        return ret;
    }
    ...
    if (_linked_to_isp)
        ...
        _cif_csi2_sd->unsubscribe_event (V4L2_EVENT_FRAME_SYNC);
    ...
    _state = CAM_HW_STATE_STOPPED;
    EXIT_CAMHW_FUNCTION();
    LOGD_CAMHW_SUBM(ISP20HW_SUBM, ">>>>>>>>>>>>>>>>>>>>>> STOPPED <<<<<<<<<<<<<<<<<<<<<<<\n");
    return ret;
}
...
XCamReturn CamHwIsp20::pause()
{
    ...
    SmartPtr<Isp20PollThread> isp20Pollthread;
    SmartPtr<BaseSensorHw> sensorHw;
    LOGD_CAMHW_SUBM(ISP20HW_SUBM, ">>>>>>>>>>>>>>>>>>>>>> PAUSE <<<<<<<<<<<<<<<<<<<<<<<\n");
    ...
    if ((mCalibDb->mfnr.enable && mCalibDb->mfnr.motion_detect_en) ||
        mCalibDb->af.ldg_param.enable) {
        if (mIspSpThread.ptr()) mIspSpThread->stop();
    }
#endif
    ...
    _state = CAM_HW_STATE_PAUSED;
    LOGD_CAMHW_SUBM(ISP20HW_SUBM, ">>>>>>>>>>>>>>>>>>>>>> PAUSED <<<<<<<<<<<<<<<<<<<<<<<\n");
    return ret;
}
...
{
    LOGD_CAMHW_SUBM(ISP20HW_SUBM, ">>>>>>>>>>>>>>>>>>>>>> RESUME <<<<<<<<<<<<<<<<<<<<<<<\n");
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    SmartPtr<Isp20PollThread> isp20Pollthread;
    ...
    if (mTnrProcUint.ptr()) {
        ret = mTnrProcUint->start();
        if (ret < 0) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "start tnr dev err: %d\n", ret);
            return ret;
        }
    }
    if (mNrProcUint.ptr()) {
        ret = mNrProcUint->start();
        if (ret < 0) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "start nr dev err: %d\n", ret);
            return ret;
        }
    }
    if (mFecProcUint.ptr()) {
        ret = mFecProcUint->start();
        if (ret < 0) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "start fec dev err: %d\n", ret);
            return ret;
        }
    }

    ret = mPollthread->start();
    if (ret < 0) {
        LOGE_CAMHW_SUBM(ISP20HW_SUBM, "start poll thread err: %d\n", ret);
        return ret;
    }
    if (mPollLumathread.ptr()) {
        ret = mPollLumathread->start();
        if (ret < 0) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "start poll luma thread err: %d\n", ret);
            return ret;
        }
    }
    if (mPollIsppthread.ptr()) {
        ret = mPollIsppthread->start();
        if (ret < 0) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "start poll ispp thread err: %d\n", ret);
            return ret;
        }
    }
    ...
    ret = sensorHw->start();
    if (ret < 0) {
        LOGE_CAMHW_SUBM(ISP20HW_SUBM, "start sensor device err: %d\n", ret);
        return XCAM_RETURN_ERROR_FAILED;
    }

    _state = CAM_HW_STATE_STARTED;
    LOGD_CAMHW_SUBM(ISP20HW_SUBM, ">>>>>>>>>>>>>>>>>>>>>> RESUMED <<<<<<<<<<<<<<<<<<<<<<<");
    return ret;
}
        ...
        SmartPtr<RkAiqIspOtherParamsProxy>& aiq_other_results)
{
    ...
    case RK_ISP2X_HDRTMO_ID:
    {
        ...
        for(int i = 0; i < 6; i++) {
            if(isnan(expo[i]) || isinf(expo[i])) {
                LOGE_CAMHW_SUBM(ISP20HW_SUBM, "expo is nan or inf!\n");
        ...
        else if(aiq_results->data()->ahdr_proc_res.MgeProcRes.sw_hdrmge_mode == 1 && !( aiq_results->data()->ahdr_proc_res.LongFrameMode))
        {
            if(nextSExpo != 0 )
                ...
                LOGE_CAMHW_SUBM(ISP20HW_SUBM, "Next short frame expo is ZERO!\n");
                return XCAM_RETURN_ERROR_PARAM;
            }
            nextRatioLM = 1;
            if( curSExpo != 0)
                ...
                LOGE_CAMHW_SUBM(ISP20HW_SUBM, "Cur short frame expo is ZERO!\n");
                return XCAM_RETURN_ERROR_PARAM;
            }
        ...
        else if(aiq_results->data()->ahdr_proc_res.MgeProcRes.sw_hdrmge_mode == 2 && !( aiq_results->data()->ahdr_proc_res.LongFrameMode))
        {
            ...
            if(nextMExpo != 0)
                ...
                LOGE_CAMHW_SUBM(ISP20HW_SUBM, "Next Middle frame expo is ZERO!\n");
                return XCAM_RETURN_ERROR_PARAM;
            }
            if(curSExpo != 0)
                curRatioLS = curLExpo / curSExpo;
            ...
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "merge mode error!\n");
            return XCAM_RETURN_ERROR_PARAM;
        }

        if(nextRatioLS < 1 || curRatioLS < 1 || nextRatioLM < 1) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "nextRatioLS:%f curRatioLS:%f nextRatioLM:%f, AE ratio < 1!\n", nextRatioLS, curRatioLS, nextRatioLM);
            return XCAM_RETURN_ERROR_PARAM;
        }
        ...
        if(aiq_results->data()->ahdr_proc_res.MgeProcRes.sw_hdrmge_mode == 1 || aiq_results->data()->ahdr_proc_res.MgeProcRes.sw_hdrmge_mode == 2) {
            LOGV_AHDR("%s: LongFrameMode:%d nextRatioLS:%f nextRatioLM:%f curRatioLS:%f\n", __FUNCTION__,
                      aiq_results->data()->ahdr_proc_res.LongFrameMode, nextRatioLS, nextRatioLM, curRatioLS);
            LOGV_AHDR("%s: nextLgmax:%f curLgmax:%f \n", __FUNCTION__, nextLgmax, curLgmax);
        }
        ...
        if(aiq_results->data()->ahdr_proc_res.MgeProcRes.sw_hdrmge_mode == 1 || aiq_results->data()->ahdr_proc_res.MgeProcRes.sw_hdrmge_mode == 2)
            LOGV_AHDR("%s: sw_hdrmge_gain0:%d sw_hdrmge_gain0_inv:%d sw_hdrmge_gain1:%d sw_hdrmge_gain1_inv:%d\n", __FUNCTION__,
        ...
        //get predict para
        bool SceneStable = hdrtmoSceneStable(frameId, &aiq_results->data()->ahdr_proc_res.TmoFlicker,
        ...
        LOGV_AHDR("%s: SceneStable:%d K_Rolgmean:%d iir:%d iir_max:%d global_tmo_strength:%d\n", __FUNCTION__,
        ...
        aiq_results->data()->ahdr_proc_res.TmoProcRes.sw_hdrtmo_set_lgrange0  = CalcSetLgrange0(&aiq_results->data()->ahdr_proc_res.TmoProcRes, lgmin, nextLgmax);

        //setlgmean
        unsigned short set_lgmean = 0;
        set_lgmean = CalcSetLgmean(frameId, &aiq_results->data()->ahdr_proc_res.Predict, &aiq_results->data()->ahdr_proc_res.TmoFlicker);
        aiq_results->data()->ahdr_proc_res.TmoProcRes.sw_hdrtmo_set_lgmean  = set_lgmean;

        //set palhpa
        aiq_results->data()->ahdr_proc_res.TmoProcRes.sw_hdrtmo_set_palpha  = aiq_results->data()->ahdr_proc_res.TmoProcRes.sw_hdrtmo_palpha_0p18;//CalcSetPalhpa(&aiq_results->data()->ahdr_proc_res.TmoProcRes, &aiq_results->data()->ahdr_proc_res.Predict, &aiq_results->data()->ahdr_proc_res.TmoFlicker, set_lgmean);

        //set lgrange1
        aiq_results->data()->ahdr_proc_res.TmoProcRes.sw_hdrtmo_set_lgrange1  = CalcSetLgrange1(&aiq_results->data()->ahdr_proc_res.TmoProcRes, lgmin, nextLgmax, set_lgmean);

        //set lgavgmax
        aiq_results->data()->ahdr_proc_res.TmoProcRes.sw_hdrtmo_set_lgavgmax  = CalcSetLgavgmax(&aiq_results->data()->ahdr_proc_res.TmoProcRes, set_lgmean);

        LOGV_AHDR("%s: sw_hdrtmo_expl_lgratio:%d sw_hdrtmo_lgscl_ratio:%d "
        ...
        LOGV_AHDR("%s: sw_hdrtmo_set_lgrange0:%d sw_hdrtmo_set_lgrange1:%d\n", __FUNCTION__,
        ...
        LOGV_AHDR("%s: sw_hdrtmo_set_lgmean:%d sw_hdrtmo_set_palpha:%d sw_hdrtmo_set_lgavgmax:%d\n", __FUNCTION__,
                  aiq_results->data()->ahdr_proc_res.TmoProcRes.sw_hdrtmo_set_lgmean,
    ...
    for (i = 0; i <= RK_ISP2X_MAX_ID; i++) {
        if (update_params->module_cfg_update & (1LL << i)) {
            ...
            case RK_ISP2X_CPROC_ID:
                ...
                break;
            case RK_ISP2X_IE_ID:
                full_params->others.ie_cfg = update_params->others.ie_cfg;
                break;
...
CamHwIsp20::setIspParamsSync(int frameId)
{
    ...
    while (!_pending_isp_other_params_queue.empty()) {
        SmartPtr<RkAiqIspOtherParamsProxy> aiqIspParamResTmp = _pending_isp_other_params_queue.front();
        if (aiqIspParamResTmp->data()->frame_id > frameId)
            break;
        _pending_isp_other_params_queue.pop_front();

        aiqIspOtherResult = aiqIspParamResTmp;
    ...
    if (!aiqIspOtherResult.ptr()) {
        LOGW_CAMHW_SUBM(ISP20HW_SUBM, "frame(%d) aiq isp other result params is null, use last result: %d\n",
                        frameId, _lastAiqIspOtherResult->data()->frame_id);
        aiqIspOtherResult = _lastAiqIspOtherResult;
    }

    // merge all isp meas params
    LOGD_CAMHW_SUBM(ISP20HW_SUBM, "start merge isp meas params\n");
    while (!_pending_isp_meas_params_queue.empty()) {
        SmartPtr<RkAiqIspMeasParamsProxy> aiqIspParamResTmp = _pending_isp_meas_params_queue.front();
        if (aiqIspParamResTmp->data()->frame_id > frameId)
            break;
        _pending_isp_meas_params_queue.pop_front();

        aiqIspMeasResult = aiqIspParamResTmp;
    ...
    if (!aiqIspMeasResult.ptr()) {
        LOGW_CAMHW_SUBM(ISP20HW_SUBM, "frame(%d) aiq isp meas result params is null, use last result: %d\n",
                        frameId, _lastAiqIspMeasResult->data()->frame_id);
        aiqIspMeasResult = _lastAiqIspMeasResult;
        aiqIspMeasResult->data()->frame_id = frameId;
        ...
        aiqIspMeasResult->data()->update_mask = RKAIQ_ISP_AHDRTMO_ID;
        ret = convertAiqMeasResultsToIsp20Params(update_params, aiqIspMeasResult,
                aiqIspOtherResult, _lastAiqIspMeasResult);
        if (ret < 0) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "rkisp1_convert_results error\n");
    ...
    if (mIspParamsDev.ptr()) {
        ...
        if (!ret) {
            ...
            *isp_params = _full_active_isp_params;
            ...
            _mutex.lock();
            // params will take effect at frame n + 1 for online mode
            if (mNormalNoReadBack && (frameId > 0)) {
                _effectingIspMeasParmMap[frameId + 1] = aiqIspMeasResult;
            } else
                _effectingIspMeasParmMap[frameId] = aiqIspMeasResult;
            _mutex.unlock();
            LOGD_CAMHW_SUBM(ISP20HW_SUBM, "ispparam id(%d) ens 0x%llx, en_up 0x%llx, cfg_up 0x%llx",
                            frameId,
                            _full_active_isp_params.module_ens,
                            _full_active_isp_params.module_en_update,
                            _full_active_isp_params.module_cfg_update);

            LOGD_CAMHW_SUBM(ISP20HW_SUBM, "device(%s) queue buffer id(%d) index %d, queue cnt %d, check exit status again[exit: %d]",
...
CamHwIsp20::setIsppMeasParams(SmartPtr<RkAiqIsppMeasParamsProxy>& isppParams)
{
    ...
    if (_state == CAM_HW_STATE_PREPARED || _state == CAM_HW_STATE_STOPPED ||
        ...
        if (mIsppParamsDev.ptr() && !mIsppParamsDev->is_activated()) {
...
XCamReturn
CamHwIsp20::setFecParams(SmartPtr<RkAiqFecParamsProxy>& fecParams)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    LOGD_CAMHW_SUBM(ISP20HW_SUBM, "set fec params");
    ...
    if (_state == CAM_HW_STATE_PREPARED || _state == CAM_HW_STATE_STOPPED ||
        _state == CAM_HW_STATE_PAUSED)
        analyzePpInitEns(fecParams);

    mFecProcUint->configParams (fecParams);
...
XCamReturn
CamHwIsp20::setTnrParams(SmartPtr<RkAiqTnrParamsProxy>& tnrParams)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    LOGD_CAMHW_SUBM(ISP20HW_SUBM, "set tnr params");
    ...
    if (_state == CAM_HW_STATE_PREPARED || _state == CAM_HW_STATE_STOPPED ||
        _state == CAM_HW_STATE_PAUSED)
        analyzePpInitEns(tnrParams);

    mTnrProcUint->configParams (tnrParams);
...
CamHwIsp20::setIsppOtherParams(SmartPtr<RkAiqIsppOtherParamsProxy>& isppOtherParams)
{
    ...
    if (_state == CAM_HW_STATE_PREPARED || _state == CAM_HW_STATE_STOPPED ||
        _state == CAM_HW_STATE_PAUSED)
        analyzePpInitEns(isppOtherParams);

    mNrProcUint->configParams (isppOtherParams);
    EXIT_CAMHW_FUNCTION();
...
XCamReturn CamHwIsp20::setSensorCrop(rk_aiq_rect_t& rect)
{
    ...
    if (!_linked_to_isp) {
        ...
        aSelection.flags = 0;
        aSelection.target = V4L2_SEL_TGT_CROP;
        aSelection.r.width = rect.width;
        aSelection.r.height = rect.height;
        aSelection.r.left = rect.left;;
        aSelection.r.top = rect.top;
        ret = mIspCoreDev->set_selection (aSelection);
        if (ret) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "set mIspCoreDev crop failed !\n");
            return ret;
...
XCamReturn CamHwIsp20::getSensorCrop(rk_aiq_rect_t& rect)
{
    ...
    if (!_linked_to_isp) {
        ...
        aSelection.flags = 0;
        aSelection.target = V4L2_SEL_TGT_CROP;
        ret = mIspCoreDev->get_selection (0, V4L2_SEL_TGT_CROP, aSelection);
        if (ret) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "get mIspCoreDev crop failed !\n");
            return ret;
        }
        rect.left = aSelection.r.left;
        rect.top = aSelection.r.top;
        rect.width = aSelection.r.width;
        rect.height = aSelection.r.height;
    }
    return ret;
}
...
void CamHwIsp20::allocMemResource(void *ops_ctx, void *config, void **mem_ctx)
{
    ...
    if (share_mem_cfg->mem_type == MEM_TYPE_LDCH) {
        ...
        *mem_ctx = (void*)(&isp20->_ldch_drv_mem_ctx);
        ...
        fecbuf_size.buf_cnt = FEC_MESH_BUF_NUM;
...
#define ALIGN(x,a)               __ALIGN_MASK(x, a-1)
    ...
    uint32_t ds_size_h = 8;

#ifndef DISABLE_ISP20SPTHREAD
    if(mIspSpThread.ptr()) {
        mIspSpThread->set_calibDb(mCalibDb);
        mIspSpThread->get_calibDb_thumbs_ds_size(&ds_size_w, &ds_size_h);
    }
#endif

    _gain_width          = (isp_src_fmt.format.width + 4 - 1) / 4;
    _gain_height         = (isp_src_fmt.format.height + ds_size_h - 1) / ds_size_h;
    _ds_width_align      = ((isp_src_fmt.format.width + ds_size_w - 1) / ds_size_w  + 1)   & (~1);
    _ds_heigth_align     = (_gain_height + 1)   & (~1);
    LOGD_CAMHW_SUBM(ISP20HW_SUBM, "set sp format: width %d %d height %d %d ds: %d %d\n",
                    _gain_width, _gain_height, _gain_width, _ds_heigth_align, ds_size_w, ds_size_h);
...
#ifndef DISABLE_ISP20SPTHREAD
    if (mIspSpThread.ptr()) {
        mIspSpThread->set_sp_dev(mIspCoreDev, mIspSpDev, _ispp_sd, mSensorDev, mLensDev, mNrProcUint);
        mIspSpThread->set_sp_img_size(_gain_width, _gain_height, _ds_width_align, _ds_heigth_align, ds_size_w, ds_size_h);
        //mIspSpThread->set_calibDb(mCalibDb);
        mIspSpThread->set_working_mode(mode);

        struct v4l2_format format;
        mIspSpDev->get_format (format);
        mIspSpThread->set_af_img_size(format.fmt.pix_mp.width, format.fmt.pix_mp.height,
...
CamHwIsp20::poll_event_ready (uint32_t sequence, int type)
{
    if (type == V4L2_EVENT_FRAME_SYNC && mIspEvtsListener) {
    ...
    } else if (type == CIFISP_V4L2_EVENT_STREAM_STOP && mIspEvtsListener) {
        SmartPtr<SensorHw> mSensor = mSensorDev.dynamic_cast_ptr<SensorHw>();
        SmartPtr<Isp20Evt> evtInfo = new Isp20Evt(this, mSensor);
        evtInfo->evt_code = type;
        evtInfo->sequence = sequence;
        evtInfo->expDelay = _exp_delay;
        return  mIspEvtsListener->ispEvtsCb(evtInfo);
    } else if (type == CIFISP_V4L2_EVENT_STREAM_START && mIspEvtsListener) {
        SmartPtr<SensorHw> mSensor = mSensorDev.dynamic_cast_ptr<SensorHw>();
        SmartPtr<Isp20Evt> evtInfo = new Isp20Evt(this, mSensor);
        evtInfo->evt_code = type;
        evtInfo->sequence = sequence;
...
XCamReturn
CamHwIsp20::getEffectiveExpParams(uint32_t id, SmartPtr<RkAiqExpParamsProxy>& expParams, uint64_t &timestamp)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    XCAM_ASSERT (mSensorDev.ptr ());
    SmartPtr<SensorHw> mSensor = mSensorDev.dynamic_cast_ptr<SensorHw> ();
    ret = mSensor->getEffectiveExpParams (expParams, id);
    if (ret < XCAM_RETURN_NO_ERROR)
        LOGE_CAMHW_SUBM(ISP20HW_SUBM, "id %d, Failed to get exposure!\n", id);

    XCAM_ASSERT (mPollthread.ptr ());
    SmartPtr<Isp20PollThread> isp20Pollthread = mPollthread.dynamic_cast_ptr<Isp20PollThread>();
    ret = isp20Pollthread->get_sof_timestamp (id, timestamp);
    if (ret < XCAM_RETURN_NO_ERROR)
        LOGE_CAMHW_SUBM(ISP20HW_SUBM, "Failed to get timestamp!\n");
...
CamHwIsp20::analyzePpInitEns(SmartPtr<cam3aResult> result)
{
    ...
    if (result->getType() == RESULT_TYPE_ATNR) {
        // TODO: tnr init_ens should be always on ?
        SmartPtr<RkAiqTnrParamsProxy> tnr = nullptr;
        tnr = result.dynamic_cast_ptr<RkAiqTnrParamsProxy>();
        if (tnr.ptr()) {
            rk_aiq_isp_tnr_t& tnr_param = tnr->data()->tnr;
            if(tnr_param.tnr_en) {
                if (tnr_param.mode > 0)
    ...
    } else if (result->getType() == RESULT_TYPE_AFEC) {
        SmartPtr<RkAiqFecParamsProxy> fec = nullptr;
        fec = result.dynamic_cast_ptr<RkAiqFecParamsProxy>();
        if (fec.ptr()) {
            rk_aiq_isp_fec_t& fec_param = fec->data()->fec;
            if(fec_param.fec_en) {
    ...
    } else if (result->getType() == RESULT_TYPE_ANR) {
        // TODO: nr,sharp init_ens always on ?
        mPpModuleInitEns |= ISPP_MODULE_SHP | ISPP_MODULE_NR;
    } else if (result->getType() == RESULT_TYPE_AORB) {
        SmartPtr<RkAiqIsppMeasParamsProxy> orb = nullptr;
        orb = result.dynamic_cast_ptr<RkAiqIsppMeasParamsProxy>();
        if (orb.ptr()) {
            rk_aiq_isp_orb_t& orb_param = orb->data()->orb;
            if(orb_param.orb_en)
