...
static int pivariety_enum_mbus_code(
			struct v4l2_subdev *sd,
...
static int pivariety_enum_framesizes(
			struct v4l2_subdev *sd,
			struct v4l2_subdev_pad_config *cfg,
...
__pivariety_get_pad_crop(struct pivariety *pivariety, struct v4l2_subdev_pad_config *cfg,
				...
				struct v4l2_subdev_selection *sel)
{
	...
	case V4L2_SEL_TGT_CROP: {
		mutex_lock(&pivariety->mutex);
		sel->r = *__pivariety_get_pad_crop(pivariety, cfg, sel->pad,
						sel->which);
		mutex_unlock(&pivariety->mutex);
				     ...
				     struct v4l2_mbus_config *cfg)
{
	...
	if (pivariety->lanes > pivariety->bus.mipi_csi2.num_data_lanes)
		return -EINVAL;
			...
			const struct i2c_device_id *id)
{
	...
	ret = v4l2_fwnode_endpoint_parse(endpoint, &pivariety->ep);
	fwnode_handle_put(endpoint);
	if (ret) {
		dev_err(dev, "Could not parse endpoint\n");
		return ret;
